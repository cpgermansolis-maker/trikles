...
## Funciones de IA (transcripción + denuncias) — ⚠️ key compartida desde el navegador
- **`transcripcion.html`** (audio→minuta) y **`denuncias.html`** (auto-llenado/evaluación/dictamen de casos) llaman a **Claude Y Gemini DIRECTO desde el navegador** (`fetch` a `api.anthropic.com`, header `anthropic-dangerous-direct-browser-access`). Usan **UNA sola API key compartida** en Script Properties (`TR_KEYS.CLAUDE`/`GEMINI`; `trGetConfig` en `Código.js` ~2414 solo la entrega a usuarios autorizados; el **SuperAdmin** las configura en `panel.html`). ⚠️ **Si esa cuenta de Anthropic se queda sin saldo, la IA se cae para TODOS los clientes** → recargar en console.anthropic.com → Plans & Billing.
- **Blindaje de errores (2026-06-18):** helper **`mensajeIAUsuario(e)`** (junto a `withRetry` en ambos archivos) traduce cualquier fallo del proveedor a un mensaje amable y **NUNCA** filtra el error crudo (request_id, "credit balance", "Anthropic") al cliente; el detalle queda en `console.error`. **Toda pantalla de IA nueva debe usar este patrón** en sus `catch` — nunca `toast(..., e.message)` con un error de proveedor. Detalle del regex: "geneRATE" contiene "rate" → usar `rate.?limit`, no `rate` suelto.

## Changelog (corto)
- **2026-06-18** — **Blindaje de errores de IA** en `transcripcion.html` y `denuncias.html`: un cliente vio el error CRUDO de Anthropic ("credit balance too low" + request_id) al agotarse el saldo. Nuevo `mensajeIAUsuario(e)` cableado en los 7 puntos de error de IA → mensaje amable, sin filtrar proveedor/request_id (detalle en consola). Commit `1e98bfc`, push a Pages (en vivo). Germán recargó créditos; instrucción explícita: **NO contactar al cliente**.
