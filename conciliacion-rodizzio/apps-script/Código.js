...
var ROLES_VALIDOS = ['admin','auditoria','cajera','host','cocina','churrasca','encargado_piso','gerente_restaurante','gerente_administrativo','observador','comprador'];
...
function setupHojas() {
  ...
  var hojas = {
    ...
    'Recetas':        ['id','empresa_id','nombre','categoria_culinaria','area','chef_responsable_email','rendimiento','unidad_rendimiento','instrucciones','uso_aplicacion','decoracion_texto','es_elaborado','costo_indirecto_pct','pct_costo_ideal','activa','foto_url','foto_origen','creada_por','creado_at','actualizado_por','actualizado_at'],
      ...
      // ---------- Módulo 2: Reservaciones ----------
      {
        ...
        quiz: [
          { pregunta:'¿Cuándo se considera una reserva "atrasada" visualmente?',
            a:'Después de 5 minutos de la hora', b:'Después de 10 minutos de la hora (tolerancia interna)', c:'Nunca', d:'Después de 1 hora',
            correcta:'b', explicacion:'10 min de tolerancia interna; la reserva se ve roja pero NO se cancela sola.' },
          { pregunta:'¿Hasta cuánto antes puede el cliente cancelar su reserva online por su cuenta?',
            a:'5 minutos', b:'30 minutos antes', c:'1 hora', d:'No puede cancelar',
            correcta:'b', explicacion:'30 min antes desde su link único de WhatsApp.' },
          { pregunta:'¿Qué hace el botón rojo "Pausar reservas"?',
            a:'Cancela todas las reservas existentes', b:'Bloquea NUEVAS reservas online del día (las existentes se respetan)', c:'Cierra el restaurante', d:'Manda WhatsApp masivo',
      ...
      // ---------- Módulo 3: Bitácora apertura ----------
      {
        ...
        quiz: [
          { pregunta:'En fines de semana, ¿cuántas bitácoras se abren al día?',
            a:'Una sola', b:'Dos: una para Desayuno y otra para Comida', c:'Tres', d:'Ninguna',
            correcta:'b', explicacion:'Vie-Dom hay dos servicios separados (Desayuno + Comida) = dos bitácoras.' },
          { pregunta:'Las reservas del día, ¿qué estado tienen al cargar la bitácora?',
            a:'Ocupada', b:'En espera (porque aún no llega físicamente el cliente)', c:'Cancelada', d:'Desocupada',
          ...
          { pregunta:'¿Qué tarifa prellena el sistema en Buffet completo Lun-Jue?',
            a:'$249 adulto', b:'$299 adulto', c:'$590 adulto', d:'Vacío, hay que capturar',
      ...
      // ---------- Módulo 4: Plano del salón ----------
      {
        ...
        quiz: [
          { pregunta:'¿Qué color y texto aparece en una mesa que acaba de salir el grupo?',
            a:'Verde "Libre"', b:'Teal/azul "✨ DESOCUPADA"', c:'Rojo "Cerrada"', d:'Gris',
          ...
          { pregunta:'En la sugerencia automática "💡 Mesa X", ¿qué criterio usa el sistema?',
            a:'Aleatorio', b:'La mesa más chica disponible que cabe al grupo (no desperdicia mesas grandes)', c:'La primera por ID', d:'La más nueva',
          ...
          { pregunta:'Si tap una mesa libre en el plano y dices "vienen 6", ¿qué pasa si la mesa es para 4?',
            a:'Lo asigna sin avisar', b:'Te pide confirmar (advertencia: "Esta mesa es para 4, ¿asignar igual?")', c:'Lo rechaza', d:'Crea otra mesa',
      ...
      // ---------- Módulo 6: CIERRE DEL SERVICIO (CRÍTICO) ----------
      {
        ...
        quiz: [
          { pregunta:'Al capturar hora_sal en una fila Ocupada, ¿qué pasa con el estado?',
            a:'Hay que cambiarlo manualmente', b:'Pasa automáticamente a "Desocupada" (azul teal)', c:'Pasa a "Cancelada"', d:'No cambia',
          ...
          { pregunta:'En fin de semana, al terminar el Desayuno (12pm), ¿qué se hace?',
            a:'Continúas capturando la Comida en la misma bitácora', b:'Cierras la bitácora de Desayuno y abres una NUEVA de Comida', c:'No haces nada', d:'Borras todo',
          ...
          { pregunta:'Una cortesía SIN nombre del autorizante en la columna AUTORIZA, ¿qué efecto tiene al cierre?',
            a:'Ninguno', b:'Bandera roja (la cajera no puede cerrar limpio hasta resolverla)', c:'Se cobra al cliente', d:'Se borra automáticamente',
          ...
          { pregunta:'¿Tu sello de cierre lo puede firmar otra persona desde su cuenta?',
            a:'Sí', b:'No: cada quien firma desde SU cuenta autenticada (queda con tu user_id y email)', c:'Solo Mónica', d:'Solo en lunes',
          ...
          { pregunta:'Una reserva quedó en "En espera" pero ya pasaron 2 horas y no llegó. ¿Qué haces antes de cerrar?',
            a:'La dejas como "En espera"', b:'La cambias a "No llegó" capturando un motivo', c:'La eliminas', d:'No haces nada',
            correcta:'b', explicacion:'Si la dejas en "En espera" suma al total. Cámbiala a "No llegó" con motivo para que NO sume.' }
        ],
        minAprobatorio: 4
      }
  ];
    ...
    // ---------- Módulo 1: Arquitectura ----------
    {
      ...
      quiz: [
        { pregunta:'¿Sobre qué plataforma corre el sistema Fogueira?',
          a:'Servidor dedicado AWS', b:'Google Apps Script (sin costo, conectado a Google Sheets)', c:'Heroku', d:'Servidor local',
          correcta:'b', explicacion:'Apps Script corre en infraestructura Google gratis, con backup automático en Drive y sandbox restrictivo.' },
        { pregunta:'En la base de datos del sistema, ¿dónde se guarda cada captura individual de la bitácora del host?',
          a:'En la hoja Bitacoras (todas en una fila JSON)', b:'En la hoja BitacoraFilas (cada fila es un registro independiente para anti-pérdida)', c:'En localStorage', d:'No se guarda',
          correcta:'b', explicacion:'BitacoraFilas tiene 1 fila por captura. Si falla un save, máximo se pierde 1 fila — no toda la bitácora.' },
        { pregunta:'¿Qué significa la filosofía de "soft-delete con auditoría" del sistema?',
          a:'Las cosas se borran físicamente al instante', b:'Nada se borra físicamente: se marca borrado_at + motivo + quién, conservando trazabilidad completa', c:'Solo el admin puede borrar', d:'Se hace backup a Excel',
          correcta:'b', explicacion:'Auditoría siempre puede revisar QUÉ se eliminó y POR QUÉ. Esto es crítico para auditoría operativa.' }
      ],
      minAprobatorio: 3
    },
    // ---------- Módulo 2: Usuarios ----------
    {
      ...
      quiz: [
        ...
        { pregunta:'¿Qué rol HEREDA todos los privilegios de admin?',
          a:'host', b:'cajera', c:'gerente_administrativo', d:'observador',
          correcta:'c', explicacion:'gerente_administrativo (Mónica) hereda todos los privilegios de admin. Por eso ella puede operar el panel admin.' },
        { pregunta:'¿Qué pasa cuando "eliminas" un usuario?',
          a:'Se borra físicamente del Sheet', b:'Se desactiva (activo=false): no puede entrar, pero su histórico se preserva', c:'Se cambia el password', d:'Pierde sus capturas',
        ...
        { pregunta:'¿Cómo se almacena la contraseña en la tabla Usuarios?',
          a:'En texto plano', b:'Como hash SHA-256 — imposible recuperar el password original, solo verificar coincidencia', c:'Encriptada con clave del usuario', d:'En sessionStorage',
          correcta:'b', explicacion:'Hashing one-way. Si el usuario olvida su password, hay que cambiarlo (no recuperarlo).' }
      ],
      minAprobatorio: 4
    },
    // ---------- Módulo 3: Configuración y tarifas ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es el formato CORRECTO de mesas_salon con zonas?',
          a:'1,2,3,4,5', b:'Salón:1:4,2:4|Terraza:30:4,31:4', c:'mesa1=4', d:'JSON aleatorio',
          correcta:'b', explicacion:'Zonas separadas por |, mesas dentro de cada zona separadas por coma.' },
        { pregunta:'¿Cómo se mantiene el histórico de tarifas cuando cambian los precios?',
          a:'Se sobrescribe el precio anterior', b:'Cada cambio = nueva fila con fecha_desde; la vigente en cualquier fecha es la más reciente hacia atrás', c:'Solo se guarda el último', d:'Se borran las viejas',
          correcta:'b', explicacion:'Versionamiento por fecha_desde permite reconstruir tarifas vigentes en cualquier fecha pasada.' },
        { pregunta:'¿Qué hace el helper matchSucursal(filaSuc, querySuc)?',
          a:'Borra sucursales', b:'Si fila.sucursal_id está vacío → aplica a CUALQUIER sucursal de la empresa (global). Si tiene id concreto → solo a esa', c:'Crea sucursales', d:'No existe',
          correcta:'b', explicacion:'Permite configuraciones globales (válidas para toda la empresa) vs específicas (una sucursal).' },
        { pregunta:'En Sheets locale es-MX, ¿qué precaución debes tomar con celdas tipo "5,6,7" (lista CSV)?',
          a:'Ninguna', b:'Prefijar con apóstrofe ("\'5,6,7") para que Sheets no las convierta en número', c:'Usar comillas dobles', d:'Usar punto y coma',
    ...
    // ---------- Módulo 4: Bootstraps y auditoría técnica ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué significa que un bootstrap sea "idempotente"?',
          a:'Que solo se puede correr una vez', b:'Que se puede correr N veces sin duplicar datos (compara por clave natural antes de insertar)', c:'Que requiere admin', d:'Que tarda mucho',
          correcta:'b', explicacion:'Comparación por clave natural (rol + texto, rol + orden, etc.) evita duplicación. Útil para reanudar cargas parciales.' },
        { pregunta:'¿Dónde queda registrado un override admin?',
          a:'En ningún lado', b:'En la hoja Sellos con flag es_override=true + motivo + identidad de quien hizo override', c:'Solo en logs', d:'Se manda email',
          correcta:'b', explicacion:'Override es excepción documentada. Auditoría revisa estos casos como señales de problema operativo si son frecuentes.' },
        { pregunta:'En BitacoraFilas, ¿qué pasa al eliminar una fila?',
          a:'Se borra físicamente para siempre', b:'Soft-delete: marca borrada_at + motivo + borrada_por; la fila sigue en el Sheet para auditoría, pero el endpoint la filtra', c:'Se duplica', d:'Se exporta a Excel',
          correcta:'b', explicacion:'Auditoría siempre puede revisar lo borrado y por qué. Recuperable manualmente si fue error.' },
        { pregunta:'¿Cuál es la mejor forma de correr un bootstrap (banco de preguntas o cursos)?',
          a:'Solo desde el editor Apps Script', b:'Desde admin.html con los botones "Cargar banco" / "Cargar cursos" (idempotente, con UI clara)', c:'Por SSH', d:'No se puede',
    ...
    // ---------- Módulo 5: Troubleshooting ----------
    {
      ...
      quiz: [
        { pregunta:'¿Por qué se quedaba la pantalla en blanco al hacer login en Chrome PC?',
          a:'Caché del navegador', b:'Chrome bloquea redirects programáticos (window.top.location.href en setTimeout) post-login porque considera que ya pasó la "user activation". Solo permite navegación con click humano', c:'Falla de internet', d:'Apps Script caído',
          correcta:'b', explicacion:'Solución: NO hacer redirect automático; mostrar botón visible que el usuario presiona (su click es user activation válida).' },
        { pregunta:'¿Por qué el token se guarda EN LA URL (?t=...) además de en sessionStorage?',
          a:'Para mostrarlo al usuario', b:'Por iOS Safari storage isolation: aísla sessionStorage entre subdominios googleusercontent.com. URL es la única forma de pasar el token entre cargas confiablemente', c:'Por seguridad', d:'No se guarda',
          correcta:'b', explicacion:'iOS no comparte sessionStorage entre subdominios. La URL sobrevive entre cargas; sessionStorage en iPhone no.' },
        { pregunta:'¿Cuáles son las 3 capas de protección anti-pérdida en BitacoraFilas?',
          a:'Solo backups en Excel', b:'1) Save por fila individual (BitacoraFilas), 2) reintentos infinitos con backoff, 3) sendBeacon emergency al cerrar página', c:'Solo localStorage', d:'Solo en memoria',
          correcta:'b', explicacion:'Triple capa de protección. Aún si fallan 2, la 3ra cubre. Máximo se pierde 1 fila individual.' },
        { pregunta:'¿Por qué un bootstrap con appendRow uno por uno se cae con timeout en 225 preguntas?',
          a:'Internet lento', b:'appendRow es ~150ms cada uno; 225 × 150ms = ~33s, que pasa el timeout de 25s del cliente. Solución: setValues en batch (1 sola operación, <1s)', c:'Apps Script tiene cuota', d:'Sheets es lento',
    ...
    // ---------- Módulo 6: Operaciones críticas (CRÍTICO) ----------
    {
      ...
      quiz: [
        { pregunta:'¿Por qué SIEMPRE se debe usar el deploymentId fijo en clasp deploy?',
          a:'No importa', b:'Las URLs públicas (links de reservas, mireserva, etc.) apuntan a ese deploymentId. Si lo cambias, todos los links existentes dejan de funcionar', c:'Es opcional', d:'Solo en producción',
          correcta:'b', explicacion:'Links en WhatsApp de clientes, en pestañas abiertas, en QRs impresos — todos apuntan al deploymentId. Cambiarlo = romperlos.' },
        { pregunta:'¿Qué pasa si re-corres setupConfiguracionFogueira() en producción?',
          a:'No pasa nada', b:'Reescribe Horarios y Configuracion. Si el cliente cambió horarios desde admin.html, los pierde. Solo correr en setup inicial', c:'Borra la base de datos', d:'Crea un backup',
          correcta:'b', explicacion:'No es idempotente para horarios. Después del setup inicial, NUNCA volver a correr en producción.' },
        { pregunta:'¿En cuáles tablas NUNCA se debe hacer borrado físico de filas?',
          a:'Solo Usuarios', b:'Bitacoras, BitacoraFilas, Conciliaciones, Sellos, Certificaciones, Reservas (todas tienen valor de auditoría/legal)', c:'Solo Configuracion', d:'Ninguna restricción',
          correcta:'b', explicacion:'Soft-delete o desactivación. Borrado físico de auditoría es peligroso y posiblemente ilegal en algunos casos (datos fiscales, compromisos con clientes).' },
        { pregunta:'Si sospechas que tu cuenta admin fue comprometida, ¿qué pasos sigues?',
          a:'No haces nada', b:'Cambias password inmediato, revisas Sellos por overrides extraños, revisas Usuarios por cuentas nuevas, revisas logs Apps Script', c:'Borras todo', d:'Apagas el servidor',
        ...
        { pregunta:'¿Cuál es la mejor regla de oro antes de hacer cualquier cambio en producción?',
          a:'Ve rápido para no aburrirte', b:'Pregúntate: "¿puedo deshacer esto si sale mal?". Si la respuesta es no, considera hacerlo en horario no operativo y con usuario de prueba primero', c:'No documentes', d:'Hazlo solo en viernes 5pm',
          correcta:'b', explicacion:'La cautela del admin protege a todo el restaurante. Una operación mal hecha puede dejar al equipo sin sistema durante horas.' }
      ],
      minAprobatorio: 4
    },
    // ---------- Módulo 7: Conciliación corta (apertura) ----------
    {
      ...
      quiz: [
        ...
        { pregunta:'Si un sello esperado de apertura falta (ej: cocina no firmó), ¿qué haces?',
          a:'Cierras el restaurante', b:'El servicio puede empezar (no es bloqueante); queda pendiente. Si la persona no podrá firmar, haces override admin con motivo', c:'Lo ignoras', d:'Llamas a la policía',
          correcta:'b', explicacion:'Apertura no bloquea operación. Override admin para casos imposibles, con motivo registrado en auditoría.' },
        { pregunta:'¿Tu sello de apertura puede firmarlo otra persona desde su sesión?',
          a:'Sí', b:'No: cada sello es autenticado por la sesión activa. Imposible firmar por otro desde pantalla normal (solo override admin con motivo)', c:'Solo Mónica', d:'Solo en lunes',
    ...
    // ---------- Módulo 8: Conciliación profunda (CRÍTICO) ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué hace el botón "Auto-llenar" del Cierre PROFUNDO?',
          a:'Llena con datos aleatorios', b:'Consolida automáticamente las bitácoras del día (suma adultos, niños, 3a edad, cortesías) en los campos de cierre', c:'Cierra el restaurante', d:'Borra todo',
          correcta:'b', explicacion:'Auto-llenar lee bitácoras del backend. Vie-Dom: Desayuno→ap, Comida→ci, Lun-Jue: Buffet→ap.' },
        { pregunta:'Una cortesía debe llevar 3 datos para evitar bandera roja al cierre. ¿Cuáles son?',
          a:'Solo el monto', b:'Autoriza (Mónica o Gabriel), folio del ticket POS, motivo en observaciones', c:'Solo el cliente', d:'Mesa, hora, descuento',
        ...
        { pregunta:'¿Cuántos depósitos a tesorería deben hacerse al día y por qué?',
          a:'Uno solo', b:'DOS: venta del día y comisiones bancarias (separados para análisis contable). Si falta uno, bandera roja', c:'Tres', d:'Ninguno',
        ...
        { pregunta:'En el arqueo ciego, hay diferencia FALTANTE de $850 sin explicación clara. ¿Qué haces?',
          a:'Firmas igual, mañana se ve', b:'NO firmas. Investiga (revisa tickets, denominaciones, posibles errores de captura). Si no se resuelve, escala a Mónica antes de firmar', c:'Le pides a la cajera que ponga $850 de su bolsa', d:'Lo borras del sistema',
        ...
        { pregunta:'Cuando firmas el cierre del servicio, ¿qué estás diciendo formalmente?',
          a:'Nada en particular', b:'Que revisaste todo, las banderas están explicadas, lo no resuelto fue escalado, y das fe de que la conciliación refleja la realidad operativa', c:'Que viste el sistema', d:'Que confías en la cajera',
          correcta:'b', explicacion:'Tu firma es un acto formal con responsabilidad. Si firmas algo no revisado y aparece un problema mañana, es responsabilidad tuya.' }
      ],
      minAprobatorio: 4
    },
    // ---------- Módulo 9: Resumen del día y banderas rojas (CRÍTICO) ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué umbral marca como bandera roja la diferencia de Arqueo ciego?',
          a:'$0 (cualquier diferencia)', b:'$200 (configurable). Si > $200 → bandera roja que requiere investigación', c:'$1000', d:'No hay umbral',
          correcta:'b', explicacion:'Umbral default $200. Diferencias mayores requieren investigar tickets, denominaciones, errores de captura.' },
        { pregunta:'Si ves la bandera "% Cancelaciones > 3%" en rojo, ¿qué haces?',
          a:'La ignoras', b:'Revisas las cancelaciones una por una y verificas que cada una tenga autorización documentada. Si alguna no la tiene, escala', c:'Cancelas más', d:'Borras del sistema',
          correcta:'b', explicacion:'Cancelaciones masivas pueden indicar errores de POS, malo servicio, o manipulación. Cada una debe estar autorizada.' },
        { pregunta:'La bandera "Saltos en folios consecutivos" es CRÍTICA porque:',
          a:'Es decorativa', b:'Indica tickets faltantes — pueden ser anulados sin documentar, perdidos, o tickets emitidos no reportados (potencial fraude)', c:'No importa', d:'Es un error visual',
          correcta:'b', explicacion:'Es la bandera más sospechosa de fraude. Cada folio faltante debe tener explicación.' },
        { pregunta:'En la sección "Conclusión del auditor", si hay banderas rojas serias sin resolver, ¿qué estatus debes seleccionar?',
          a:'OK, así puedo irme', b:'"Banderas críticas" — NO firmar hasta escalar a Mónica/dirección y documentar evidencia', c:'Cualquiera', d:'Lo que diga la cajera',
        ...
        { pregunta:'Si descubres una cortesía firmada CON TU NOMBRE pero TÚ no la autorizaste, ¿qué haces?',
          a:'La dejas pasar', b:'NO firmas la conciliación. Documentas evidencia (foto, captura) y escalas inmediato a Mónica/dirección — es fraude operativo grave', c:'La firmas para regularizar', d:'La borras del sistema',
          correcta:'b', explicacion:'Cortesía con tu nombre que tú no firmaste = fraude. NUNCA encubrir. Escalar inmediato con evidencia.' }
      ],
      minAprobatorio: 4
    },
    // ---------- Módulo 10: Recetas y costeo ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué campo determina el costo real por unidad de un ingrediente?',
          a:'ultimo_costo', b:'precio_real_unitario (precio de la última compra real, en MXN por unidad_base)', c:'costo_promedio', d:'merma_pct',
          correcta:'b', explicacion:'precio_real_unitario es el que usa el sistema para calcular el costo en vivo de las recetas. Se puede actualizar manualmente o vía importación SR12.' },
        { pregunta:'¿Qué hace el factor_rendimiento en el cálculo del costo?',
          a:'Nada', b:'Divide el precio entre lo que realmente termina en el plato (descuenta la merma no aprovechable). A menor factor → mayor costo real', c:'Multiplica el precio por el rendimiento de la receta', d:'Es decorativo',
          correcta:'b', explicacion:'factor_rendimiento = 1 − merma_no_aprovechable_pct. Ingrediente con 20% de merma no aprovechable tiene factor 0.80 → el costo sube ~25% vs precio bruto.' },
        { pregunta:'¿Qué rol puede editar precios y mermas de ingredientes directamente (sin propuesta)?',
          a:'Solo el chef', b:'Admin, Gerente Administrativo y Comprador (Weslley)', c:'Cualquiera', d:'Solo Auditoría',
        ...
        { pregunta:'Cuando un chef propone una receta nueva desde la charola, ¿qué pasa con el costo?',
          a:'Se activa de inmediato y cambia el costo', b:'Queda en pendiente — alguien con rol admin/gte_adm debe aprobarla antes de que entre al cálculo de costos', c:'El chef puede activarla él solo', d:'Se borra automáticamente',
          correcta:'b', explicacion:'Modelo B de autorización: propuesta → revisión admin → activación. Ningún cambio llega a costos sin revisión.' }
      ],
      minAprobatorio: 3
    },
    // ---------- Módulo 11: Inventario Churrasca ----------
    {
      ...
      quiz: [
        { pregunta:'¿Con qué frecuencia se hace el inventario formal de churrasca?',
          a:'Diario', b:'Semanal (Lun–Dom). El cierre del domingo es el inicial del lunes siguiente', c:'Mensual', d:'Cuando Marcos quiera',
          correcta:'b', explicacion:'Ciclo semanal. Lunes = conteo inicial, Domingo = conteo final. La diferencia entre ambos más entradas/salidas da el cuadre.' },
        { pregunta:'Si el inventario teórico dice que debería haber 50 kg de picaña y el físico dice 43 kg, ¿qué indica?',
          a:'Sobrante de 7 kg', b:'Faltante de 7 kg — se consumió o perdió más de lo registrado (merma no capturada, error de captura, o algo más grave)', c:'Inventario cuadrado', d:'Error del sistema',
          correcta:'b', explicacion:'Faltante = Físico < Teórico. Causas: merma sin reportar, entrada sin capturar, error de conteo, o producto que sale sin registrar.' },
        { pregunta:'¿Qué es una "entrada" en el inventario churrasca?',
          a:'El inicio de sesión del churrasquero', b:'Mercancía que llega a la zona: compras, recepciones (carnes, carbón, etc.)', c:'Un cliente nuevo', d:'Una espada servida',
          correcta:'b', explicacion:'Entrada = algo físico que entra al inventario (compra, recepción). Sale = lo que se usa en servicio. Diferencia = lo que falta sin explicación.' }
      ],
      minAprobatorio: 3
    },
    // ---------- Módulo 12: Importar SR12 ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué hace el dry-run antes de aplicar la importación SR12?',
          a:'Borra todos los costos actuales', b:'Muestra el preview completo de qué cambiaría SIN modificar nada en la base de datos — seguro de correr', c:'Aplica los cambios directamente', d:'Nada',
          correcta:'b', explicacion:'Dry-run = ensayo sin consecuencias. Siempre hacerlo primero para detectar matches incorrectos o precios absurdos antes de que impacten costos.' },
        { pregunta:'¿Qué son los "huérfanos Fogueira"?',
          a:'Ingredientes eliminados', b:'Ingredientes en el catálogo Fogueira que no tienen match con ningún producto de SR12 (sub-recetas, preparados internamente, o ya no se compran)', c:'Archivos corruptos', d:'Clientes sin reserva',
          correcta:'b', explicacion:'Huérfanos Fogueira no reciben actualización de costos de SR12. Si son insumos reales, conviene vincularlos manualmente.' },
        { pregunta:'Si después de aplicar la importación notas que un ingrediente quedó vinculado al producto incorrecto, ¿qué haces?',
          a:'No pasa nada', b:'Usas la función de Reversa para deshacer la importación, corriges el vínculo manualmente, y vuelves a importar', c:'Editas el costo a mano', d:'Deshabilitas el ingrediente',
          correcta:'b', explicacion:'Reversa restaura los costos anteriores. Después de corregir el match (campo clave_sr12 o IngredientesSR12Match), la siguiente importación cuadrará bien.' },
        { pregunta:'¿Cuántos archivos XLS espera el módulo de importación?',
          a:'1', b:'Hasta 6 (uno por almacén de SoftRestaurant: General, Cocina, Churrasca, Bar, Frío, Seco)', c:'Cualquier cantidad', d:'Solo 2',
    ...
    // ---------- Módulo 13: Flujo de conciliación ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuántos depósitos a tesorería se deben hacer por servicio y para qué?',
          a:'Solo uno', b:'DOS: Depósito 1 (venta del día) y Depósito 2 (comisiones bancarias). Se separan para análisis contable diferenciado', c:'Los que quiera la cajera', d:'Ninguno',
          correcta:'b', explicacion:'Reglas Fogueira: separación contable obligatoria. Sin los dos → bandera roja, no se puede firmar el cierre.' },
        { pregunta:'El indicador "Δ Host vs POS" mide:',
          a:'La diferencia de tiempo entre turnos', b:'La diferencia entre comensales registrados por el host en bitácora vs personas registradas en tickets del POS. Debe ser cercano a cero', c:'La diferencia de sueldo entre host y cajera', d:'El tiempo del servicio',
          correcta:'b', explicacion:'Si no cuadran → alguien cobró sin registrar en bitácora, o el host capturó sin que hubiera cobro. Es el cruce más importante anti-fraude.' },
        { pregunta:'¿Qué es el "arqueo ciego" y por qué se llama así?',
          a:'La cajera cuenta con los ojos cerrados', b:'El sistema calcula el teórico sin que la cajera sepa el número; ella cuenta lo físico sin saber qué espera el sistema. Al comparar, ninguno pudo "ajustar" al otro', c:'Un formato de Excel', d:'Un turno nocturno',
          correcta:'b', explicacion:'El "ciego" es intencional: si la cajera supiera el teórico, podría ajustar el conteo. Así el cruce es honesto e independiente.' }
      ],
      minAprobatorio: 3
    },
    // ---------- Módulo 14: Mi manual (El instructivo) ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es la diferencia entre el Curso y el Manual (instructivo)?',
          a:'Son lo mismo', b:'El Curso es para aprender y certificarse (módulos en secuencia, ~90 min). El Manual es para consulta rápida durante la operación (1-2 min por tema)', c:'El manual es más largo', d:'El manual tiene examen',
          correcta:'b', explicacion:'Herramientas distintas para momentos distintos. El curso forma; el manual consulta. Ambos son necesarios.' },
        { pregunta:'Como admin, ¿qué secciones del instructivo puedes ver?',
          a:'Solo la sección de admin', b:'TODAS las secciones de todos los roles (host, cocina, cajera, gerentes, FAQ, soporte) — para poder diagnosticar y dar soporte a cualquier usuario', c:'Solo FAQ y soporte', d:'Ninguna',
          correcta:'b', explicacion:'data-roles="admin" incluye admin en todas las secciones. Conocer el instructivo de cada rol es clave para dar soporte efectivo.' },
        { pregunta:'Un host te llama porque "no puede cerrar la bitácora". ¿Cuál es tu primer paso?',
          a:'Reiniciar el servidor', b:'Abrir el instructivo sección Host y revisar los pasos del cierre — en el 80% de los casos es un paso operativo que se saltó (no un bug)', c:'Pedirle que espere hasta mañana', d:'Borrar la bitácora',
          correcta:'b', explicacion:'La mayoría de reportes de soporte son pasos operativos omitidos, no bugs. El instructivo del rol del usuario es tu primera herramienta de diagnóstico.' }
      ],
      minAprobatorio: 3
    }
  ];
...
function modulosCursoGerenteAdministrativo() {
  return [
    // ---------- Módulo 1: Tu rol como Gerente Administrativo ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué rol del sistema HEREDA todos los privilegios de admin (acceso técnico completo)?',
          a:'host', b:'gerente_administrativo (TU rol)', c:'cajera', d:'observador',
          correcta:'b', explicacion:'gerente_administrativo (Mónica) hereda admin completo. Por eso puedes operar todo el panel de Configuración.' },
        { pregunta:'¿Qué casos SIEMPRE debes escalar a la dirección (Germán)?',
          a:'Cualquier cosa pequeña', b:'Pérdidas materiales >$5K, reclamo legal/sanitario, sospecha de fraude, decisiones financieras grandes, problemas técnicos serios', c:'Solo si la cajera te lo pide', d:'Nunca, decides todo',
          correcta:'b', explicacion:'Lo legal, financiero grande o que dañe imagen va a dirección. Lo operativo del día tú lo manejas con tu autoridad.' },
        { pregunta:'¿Quiénes te reportan operativamente?',
          a:'Solo la cajera', b:'Gabriel (Gerente Restaurante) y a través de él: encargado de piso, hosts, cocina, cajera, churrasca. Auditoría también está bajo tu radar', c:'Solo los hosts', d:'Nadie',
          correcta:'b', explicacion:'Eres mano derecha de dirección con autoridad sobre gerentes y todo el equipo operativo.' }
      ],
      minAprobatorio: 3
    },
    // ---------- Módulo 2: Gestión de usuarios ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué pasa al "desactivar" un usuario en lugar de eliminarlo?',
          a:'Se borra físicamente', b:'Se desactiva (no puede entrar) pero su histórico se preserva. Reactivable después', c:'Pierde sus capturas', d:'Se cambia el rol',
          correcta:'b', explicacion:'NUNCA borrar físicamente. Soft-delete con auditoría es la regla — si vuelve, reactivas y mantiene historial.' },
        { pregunta:'Un usuario reprobó las 3 veces el examen. ¿Cuándo es válido resetear sus intentos?',
          a:'Cuando el usuario te lo pida', b:'SOLO después de capacitación real (con repaso del manual y curso). Sin capacitación, perderías el sentido del examen', c:'Cualquier viernes', d:'Nunca',
          correcta:'b', explicacion:'Resetear sin capacitación = oportunidades infinitas. La regla es: capacitar primero, resetear después.' },
        { pregunta:'Si una host se va por temporada (3 meses), ¿qué haces con su cuenta?',
          a:'La eliminas físicamente', b:'La desactivas (mantienes histórico). Si vuelve, reactivas en 1 click', c:'Cambias su contraseña', d:'No haces nada',
        ...
        { pregunta:'¿Cómo se almacenan las contraseñas en el sistema?',
          a:'Texto plano', b:'Hash SHA-256 (one-way). Imposible recuperar el password original; solo verificar coincidencia. Si olvida, hay que cambiarlo (no recuperarlo)', c:'Encriptadas reversibles', d:'En sessionStorage',
          correcta:'b', explicacion:'Hashing one-way para seguridad. Si olvida, le pones nueva contraseña — no se puede recuperar la vieja.' }
      ],
      minAprobatorio: 4
    },
    // ---------- Módulo 3: Configuración del restaurante (énfasis pedido) ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es el formato CORRECTO para definir mesas con zonas en la configuración?',
          a:'1,2,3,4', b:'Salón:1:4,2:4|Terraza:30:4,31:6', c:'mesa1=4', d:'JSON',
        ...
        { pregunta:'La clave `cupo_por_servicio` (default 50), ¿qué controla exactamente?',
          a:'Aforo físico total', b:'Cuántas RESERVAS ONLINE se aceptan máximo por servicio. Walk-ins NO se cuentan aquí (aforo aparte)', c:'Total de comensales del día', d:'Mesas activas',
          correcta:'b', explicacion:'Cupo es solo de reservas online. Walk-ins suman aparte hasta el aforo_fisico (92).' },
        { pregunta:'¿En qué momento NUNCA debes cambiar horarios operativos?',
          a:'Cualquier momento', b:'Durante un servicio activo (afecta reservas en vuelo). Hazlo en horario no operativo', c:'Solo en lunes', d:'Solo de noche',
        ...
        { pregunta:'Si cambias el valor de `gerente_administrativo_nombre` (de Mónica a otro nombre), ¿qué pasa con las cortesías históricas?',
          a:'Se reescriben con el nuevo nombre', b:'Se mantienen con el nombre anterior (auditoría preserva el dato del momento de la firma)', c:'Se borran', d:'Se duplican',
          correcta:'b', explicacion:'Las cortesías ya firmadas son evidencia histórica — no se reescriben. Solo las nuevas usarán el nombre actualizado.' },
        { pregunta:'¿Por qué el `umbral_grupo_grande` existe (default 10)?',
          a:'Para discriminar grupos', b:'Grupos >N personas requieren preparación especial (mesas juntas, personal). NO se confirman automáticamente; quedan en pendiente_confirmacion para que host hable con cliente', c:'Para subir el precio', d:'Para no aceptarlos',
          correcta:'b', explicacion:'Confirmar automáticamente un grupo de 15 personas sin verificar capacidad real puede causar problema operativo. Pendiente_confirmacion permite intervención humana.' }
      ],
      minAprobatorio: 4
    },
    // ---------- Módulo 4: Tarifas y promociones ----------
    {
      ...
      quiz: [
        { pregunta:'En el sistema de tarifas, cuando cambias un precio, ¿qué pasa con el precio anterior?',
          a:'Se sobrescribe', b:'Se mantiene en su fila histórica; se crea una NUEVA fila con fecha_desde actualizada. La vigente en cualquier fecha es la más reciente hacia atrás', c:'Se borra', d:'Se duplica',
        ...
        { pregunta:'¿Por qué la columna `dias_semana` debe tener valor con apóstrofe ("\'1,2,3,4")?',
          a:'Por estética', b:'Sheets locale es-MX interpreta "1,2,3,4" como número 1234. El apóstrofe lo fuerza a texto', c:'Por seguridad', d:'No tiene importancia',
          correcta:'b', explicacion:'Bug clásico de locale español. Sin apóstrofe, los días se corrompen y las tarifas se aplican mal.' },
        { pregunta:'Tarifa actual Fogueira para Buffet completo Lun-Jue, adulto:',
          a:'$249', b:'$590', c:'$299', d:'$890',
          correcta:'b', explicacion:'$590 adulto. Niño 6-10 $249. 3a edad $590. 0-5 gratis. 11+ tarifa adulto.' },
    ...
    // ---------- Módulo 5: Cortesías y autorizaciones (TU FIRMA) ----------
    {
      ...
      quiz: [
        { pregunta:'¿Quiénes son los DOS únicos autorizados para firmar cortesías en Fogueira?',
          a:'Cualquier gerente', b:'Gerente Administrativo (TÚ — Mónica) y Gerente de Restaurante (Gabriel)', c:'La cajera y el host más antiguo', d:'Solo tú',
          correcta:'b', explicacion:'Solo Mónica (tú) y Gabriel autorizan. Es el control más importante para evitar fraude operativo.' },
        { pregunta:'Una cortesía DEBE llevar 3 datos para evitar bandera roja al cierre. ¿Cuáles?',
          a:'Solo el monto', b:'Autoriza (tu nombre o de Gabriel), folio del ticket POS, motivo en observaciones', c:'Solo nombre del cliente', d:'Mesa, hora, descuento',
        ...
        { pregunta:'Al revisar el Resumen ves una cortesía con TU NOMBRE que tú NO recuerdas haber firmado. ¿Qué haces?',
          a:'La dejas pasar', b:'PARAS la firma. Investigas (host, folio, cajera). Si no se explica → fraude operativo: escala a Germán inmediato con evidencia', c:'La firmas para regularizar', d:'La borras',
          correcta:'b', explicacion:'Esta es la peor bandera. Cada cortesía con tu nombre debe ser tuya. Si no, escalar inmediato a dirección.' },
        { pregunta:'¿Las cortesías de niños 0-5 años requieren tu firma?',
          a:'Sí', b:'No, son cortesía automática por edad. El host selecciona "Bebé/niño 0-5" en el dropdown', c:'Solo en domingo', d:'Solo arriba de 3 niños',
        ...
        { pregunta:'Un host te dice: "El cliente pide descuento porque viene mucho". ¿Qué haces?',
          a:'Firmas sin pensar', b:'Aplicas tu juicio. Si tú lo conoces y consideras justificado, sí (excepcional). Si no hay política formal, NO firmas y le explicas al cliente cordialmente', c:'Llamas al chef', d:'Le pasas la firma a la cajera',
    ...
    // ---------- Módulo 6: Supervisión gerencial y override ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuándo es válido hacer un override admin de sello?',
          a:'Cuando la cajera no quiere firmar', b:'Solo cuando es REALMENTE imposible que la persona firme (turno cambió, salió enferma, falla de internet) — y SIEMPRE con motivo escrito', c:'Para saltarse controles', d:'Cualquier momento',
          correcta:'b', explicacion:'Override = excepción documentada para casos imposibles. Si la persona puede firmar, debe firmar ella.' },
        { pregunta:'¿Qué casos SIEMPRE escalas a la dirección (Germán)?',
          a:'Solo problemas técnicos', b:'Pérdidas >$5K, reclamo legal/sanitario, fraude operativo confirmado, decisiones financieras grandes, problemas técnicos serios, conflictos serios de personal', c:'Solo si el cliente reclama', d:'Nunca, decides todo',
          correcta:'b', explicacion:'Lo legal, financiero grande o con impacto en imagen va a dirección. Lo operativo del día tú lo manejas.' },
        { pregunta:'Si Gabriel está autorizando demasiadas cortesías sin patrón claro, ¿cómo intervienes?',
          a:'Lo despides', b:'En privado, con respeto. Te sientas con él, revisan juntos las cortesías, identifican el patrón. Si no se corrige, escalas', c:'Lo reportas en redes sociales', d:'No haces nada',
        ...
        { pregunta:'Si tú haces muchos overrides admin en pocas semanas, ¿qué señala?',
          a:'Que eres eficiente', b:'Posible problema operativo: el equipo no firma a tiempo, hay rotación, falta capacitación. Auditoría puede observarlo', c:'Nada, es normal', d:'Que eres importante',
    ...
    // ---------- Módulo 7: Conciliación apertura ----------
    {
      ...
      quiz: [
        { pregunta:'En fines de semana, ¿cuántas conciliaciones se abren por día?',
          a:'1 (todo el día)', b:'2 (Desayuno y Comida — cada una con su propia apertura/cierre)', c:'3', d:'Ninguna',
          correcta:'b', explicacion:'Vie-Dom hay dos servicios independientes con tarifas distintas, cada uno con su conciliación.' },
        { pregunta:'¿Las tarifas en la pestaña Apertura se capturan a mano cada día?',
          a:'Sí', b:'No: se prellenan automáticamente leyendo la fila vigente de la tabla Tarifas para esa fecha', c:'Solo en sábado', d:'Solo si la cajera lo pide',
          correcta:'b', explicacion:'Versionamiento de tarifas con fecha_desde permite reconstruir tarifas correctas en cualquier fecha.' },
        { pregunta:'¿Qué pasa si falta un sello esperado al inicio del servicio?',
          a:'Bloquea la operación', b:'NO bloquea — el servicio puede empezar. Queda como pendiente. Si la persona no podrá firmar, override admin con motivo', c:'Cancela la conciliación', d:'Se firma solo',
          correcta:'b', explicacion:'Apertura no bloquea operación. Override admin para casos imposibles, con motivo registrado.' },
        { pregunta:'Como hábito, ¿con cuánto tiempo de anticipación deberías llegar a la apertura?',
          a:'5 minutos', b:'15-20 min antes (verificar tarifas, fondo, sellos cocina/churrasca, config sin cambios)', c:'1 hora', d:'No importa',
    ...
    // ---------- Módulo 8: Conciliación profunda ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué hace "Auto-llenar" en el Cierre PROFUNDO?',
          ...
          correcta:'b', explicacion:'Auto-llenar lee bitácoras del backend. Vie-Dom: Desayuno→ap, Comida→ci. Lun-Jue: Buffet→ap.' },
        { pregunta:'¿Cuántos depósitos a tesorería deben hacerse al día y por qué?',
          a:'Uno solo', b:'DOS: venta del día + comisiones bancarias (separados para análisis contable)', c:'Tres', d:'Ninguno',
          correcta:'b', explicacion:'Reglas Fogueira: separación contable obligatoria. Auditoría revisa que ambos estén con folio.' },
        { pregunta:'En arqueo ciego hay diferencia FALTANTE de $850 sin explicación clara. ¿Qué haces?',
          a:'Firmas igual', b:'NO firmas. Investigas (revisa tickets, denominaciones, errores de captura). Si no se resuelve, escala a Germán antes de firmar', c:'Le pides a la cajera $850 de su bolsa', d:'Borras del sistema',
          correcta:'b', explicacion:'$850 sin explicación NO es residual. Diferencias > $200 deben investigarse; > $1000 escalar inmediato.' },
        { pregunta:'Cuando firmas el cierre del servicio, ¿qué estás diciendo formalmente?',
          a:'Nada', b:'Que revisaste todo, las banderas están explicadas, lo no resuelto fue escalado, y das fe de que la conciliación refleja la realidad', c:'Solo viste el sistema', d:'Confías en la cajera',
          correcta:'b', explicacion:'Tu firma es acto formal con responsabilidad. Si firmas no revisado y aparece problema mañana, es responsabilidad tuya.' },
        { pregunta:'Una cortesía debe llevar 3 datos para evitar bandera roja. ¿Cuáles?',
          a:'Solo el monto', b:'Autoriza (Mónica o Gabriel), folio del ticket POS, motivo en observaciones', c:'Solo cliente', d:'Mesa, hora, descuento',
          correcta:'b', explicacion:'Estos 3 datos garantizan trazabilidad: quién aprobó + cruce con POS + justificación.' }
      ],
      minAprobatorio: 4
    },
    // ---------- Módulo 9: Resumen del día (CRÍTICO) ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué umbral marca como bandera roja la diferencia de Arqueo ciego?',
          a:'$0 (cualquier)', b:'$200 (configurable). Si > $200 → bandera roja que requiere investigación', c:'$1000', d:'No hay',
          correcta:'b', explicacion:'Umbral default $200. Diferencias mayores requieren investigar tickets, denominaciones, errores.' },
        { pregunta:'¿Por qué la bandera "Saltos en folios consecutivos" es la más CRÍTICA?',
          a:'Es decorativa', b:'Indica tickets faltantes — anulados sin documentar, perdidos, o emitidos no reportados (potencial fraude)', c:'No importa', d:'Es error visual',
        ...
        { pregunta:'Si descubres una cortesía con TU NOMBRE pero TÚ NO la firmaste, ¿qué haces?',
          a:'La dejas pasar', b:'NO firmas la conciliación. Documentas evidencia (foto, captura) y escalas inmediato a Germán — es fraude operativo grave', c:'La firmas para regularizar', d:'La borras del sistema',
          correcta:'b', explicacion:'Cortesía con tu nombre que tú no firmaste = fraude. NUNCA encubrir. Escalar inmediato con evidencia.' },
        { pregunta:'En la Conclusión del auditor, si hay banderas rojas serias sin resolver, ¿qué estatus seleccionas?',
          a:'OK', b:'"Banderas críticas" — NO firmar hasta escalar a Germán/dirección y documentar evidencia', c:'Cualquiera', d:'Lo que diga la cajera',
          correcta:'b', explicacion:'El estatus debe reflejar la realidad. Firmar "OK" con banderas críticas = responsabilidad tuya cuando aparezca el problema.' },
    ...
    // ---------- Módulo 10: Auditoría operativa mensual ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué tipo de problemas se ven mejor con vista MENSUAL que diaria?',
          a:'Solo banderas obvias', b:'Patrones repetidos: cortesías recurrentes, faltantes pequeños sistemáticos, overrides frecuentes — señales que de a uno parecen normales pero juntas son sospechosas', c:'Los que pasan en lunes', d:'Ninguno',
          correcta:'b', explicacion:'Patrones se ven con vista mensual. Faltantes de $50 cada lunes 4 semanas seguidas = $200 totales y posible sustracción sistemática.' },
        { pregunta:'En tu auditoría mensual de cortesías, ¿qué señal de alarma debes buscar?',
          a:'Que haya cortesías', b:'Que un host acumule muchas (ej: 20% del total mensual), o que un cliente se repita, o motivos sin variedad ("queja, queja, queja")', c:'Que el monto sea alto', d:'Solo total mensual',
        ...
        { pregunta:'Si haces muchos overrides admin en el mes, ¿qué señal es?',
          a:'Que eres eficiente', b:'Posible proceso roto: equipo no firma a tiempo, falta capacitación, alta rotación. Causa raíz vs síntoma', c:'Nada', d:'Que eres importante',
        ...
        { pregunta:'¿Cuál es el valor de detectar problemas en auditoría INTERNA mensual vs auditoría externa?',
          a:'Ninguno', b:'Diferencia gigantesca: interno = corrección barata (capacitación, ajuste). Externo = costo alto (multa, demanda, daño reputacional)', c:'Solo cosmético', d:'Igual',
...
function modulosCursoCajera() {
  return [
    // ---------- Módulo 1: Tu rol como cajera ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué decisiones financieras tú como cajera NO autorizas?',
          a:'Ninguna, eres la jefa de caja', b:'Cortesías (las firma Mónica o Gabriel), cancelaciones masivas, descuentos no documentados, devoluciones grandes', c:'Solo cortesías', d:'Solo el cambio',
          correcta:'b', explicacion:'Tu autoridad es operativa, no decisoria. Solo Mónica y Gabriel autorizan cortesías y excepciones financieras.' },
        { pregunta:'Si un cliente te presiona para "autorizar" un descuento sin aprobación, ¿qué haces?',
          a:'Lo autorizas para no quedar mal', b:'NO lo autorizas. Llamas a Mónica o Gabriel. Tu trabajo es procesar pagos según las reglas, no decidir excepciones', c:'Le pides identificación', d:'Le cobras de tu bolsa',
        ...
        { pregunta:'¿Hasta qué hora dura tu sesión iniciada en el sistema?',
          a:'1 hora', b:'8 horas', c:'Hasta las 3:00 am del día siguiente (día lógico restaurante)', d:'Hasta medianoche',
    ...
    // ---------- Módulo 2: Apertura ----------
    {
      ...
      quiz: [
        { pregunta:'Al recibir el fondo de cambio, ¿qué buena práctica debes seguir?',
          a:'Aceptar lo que te dan sin contar', b:'Contar delante de Mónica/quien entrega, verificar denominaciones, firmar recibido como evidencia', c:'Solo contar al final', d:'Pedir más siempre',
        ...
        { pregunta:'Tu sello de apertura, ¿lo puede firmar otra cajera por ti desde su sesión?',
          a:'Sí, son del mismo equipo', b:'No: cada sello es autenticado por la sesión activa. Solo tu cuenta puede firmar tu sello (excepto override admin con motivo)', c:'Solo Mónica', d:'Solo en domingo',
        ...
        { pregunta:'Si el fondo de cambio viene con un monto DISTINTO al declarado, ¿qué haces?',
          a:'Firmas recibido y aclaras después', b:'NO firmas recibido sin que se aclare. Notificas a Mónica inmediato', c:'Cuentas otra vez y pones lo que falta', d:'Lo aceptas y empiezas',
          correcta:'b', explicacion:'Diferencia al inicio = problema desde la primera hora. Aclarar antes de firmar previene que la diferencia recaiga sobre ti.' },
        { pregunta:'Si tienes urgencia y un cliente entra antes de la apertura del sistema, ¿qué haces?',
          a:'Lo dejas esperar 15 min', b:'Atiendes al cliente primero. Capturas apertura en cuanto puedas. Si pasó mucho tiempo, anota hora real en observaciones', c:'Cierras el restaurante', d:'Pides ayuda al host',
    ...
    // ---------- Módulo 3: Cobros durante el servicio ----------
    {
      ...
      quiz: [
        { pregunta:'Antes de entregar el cambio al cliente, ¿qué debes hacer?',
          a:'Confiar en el POS y entregar', b:'Contar el cambio en voz alta o silenciosamente, verificar monto antes de entregar', c:'Pedir al cliente que cuente', d:'Nada',
          correcta:'b', explicacion:'Contar dos veces evita errores. Tu responsabilidad incluye la precisión del cambio.' },
        { pregunta:'Si abres el cajón sin venta más de 2 veces sin justificación clara, ¿qué pasa?',
          a:'Nada', b:'Bandera roja en el resumen del día. Mónica te preguntará por cada no-sale al cierre', c:'Te despiden', d:'Se cierra el cajón',
        ...
        { pregunta:'Para cancelar un ticket de cobro, ¿qué debes hacer?',
          a:'Cancelar tú sola sin avisar', b:'1) Notificar a Mónica o Gabriel, 2) Cancelar en POS, 3) Capturar motivo, 4) Anotar quién autorizó. Sin autorización = bandera roja gravísima', c:'Borrar el ticket', d:'Cobrarlo de tu bolsa',
        ...
        { pregunta:'Si la tarjeta del cliente es rechazada, ¿cómo respondes?',
          a:'Lo regañas', b:'Avisas cordialmente "la transacción no se procesó", sugieres otra tarjeta o efectivo. NO insistes ni juzgas (hay razones legítimas)', c:'Le quitas la tarjeta', d:'Le pides que se vaya',
    ...
    // ---------- Módulo 4: Captura de cortesías ----------
    {
      ...
      quiz: [
        { pregunta:'¿Quiénes son los DOS únicos autorizados para firmar cortesías en Fogueira?',
          a:'Cualquier gerente', b:'Gerente Administrativo (Mónica) y Gerente de Restaurante (Gabriel)', c:'La cajera', d:'El host más antiguo',
          correcta:'b', explicacion:'Solo Mónica y Gabriel autorizan cortesías. Tú las capturas pero NO las autorizas.' },
        { pregunta:'Una cortesía DEBE llevar 3 datos para evitar bandera roja al cierre. ¿Cuáles?',
          a:'Solo el monto', b:'Autoriza (Mónica o Gabriel), folio del ticket POS, motivo en observaciones', c:'Solo cliente', d:'Mesa, hora, descuento',
          correcta:'b', explicacion:'Estos 3 garantizan trazabilidad: quién aprobó + cruce con POS + justificación.' },
        { pregunta:'Un host te dice "Mónica autorizó cortesía" pero tú no la viste. ¿Qué haces?',
          a:'Capturas confiando en el host', b:'PARAS. Llamas a Mónica directamente para verificar. Si dice sí → captura. Si dice no → fraude operativo, notificas inmediato', c:'La capturas y luego preguntas', d:'La rechazas',
        ...
        { pregunta:'¿Las cortesías de niños 0-5 años requieren autorización de gerente?',
          a:'Sí, siempre', b:'No, son cortesía automática por edad. El host selecciona "Bebé/niño 0-5" en el dropdown', c:'Solo en domingo', d:'Solo arriba de 3 niños',
    ...
    // ---------- Módulo 5: Cierre profundo (CRÍTICO) ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuántos depósitos a tesorería deben hacerse al día y por qué?',
          a:'Uno solo', b:'DOS: venta del día + comisiones bancarias (separados para análisis contable). Si falta uno, bandera roja', c:'Tres', d:'Ninguno',
        ...
        { pregunta:'En arqueo ciego hay diferencia FALTANTE de $1,200 sin explicación. ¿Qué haces?',
          a:'Firmas igual', b:'NO firmas. Llamas a Mónica antes de cerrar. Diferencias > $1,000 escalan inmediato', c:'Pones el dinero de tu bolsa', d:'Borras del sistema',
        ...
        { pregunta:'Si un sello esperado falta al cierre (cocina no firmó), ¿quién hace el override?',
          a:'Tú como cajera', b:'Mónica o Gabriel (admin/gerente_administrativo) — tú NO tienes ese privilegio', c:'El host', d:'Cualquiera',
        ...
        { pregunta:'¿Qué información valida tu sello de cierre formalmente?',
          a:'Nada en particular', b:'Que el corte es correcto, tarjetas cuadran, cortesías autorizadas, depósitos hechos, arqueo razonable, sellos firmados', c:'Solo que viste el sistema', d:'Que confías en otros',
          correcta:'b', explicacion:'Tu firma es acto formal con responsabilidad. Si firmas no revisado, problemas mañana son tu responsabilidad.' },
        { pregunta:'¿Cuál es la mejor práctica al contar el efectivo del corte?',
          a:'Confía en el sistema', b:'Cuenta dos veces cada denominación. Si te confundes, vuelve a empezar. Si hay diferencia, recuenta antes de capturar', c:'Solo cuenta una vez', d:'Pídele al host que cuente',
    ...
    // ---------- Módulo 6: Resumen y banderas (CRÍTICO) ----------
    {
      ...
      quiz: [
        { pregunta:'En el Resumen del día, ¿quién tiene el rol PRINCIPAL en la bandera "Δ Arqueo ciego > $200"?',
          a:'El host', b:'TÚ (la cajera) — es tu cierre de efectivo vs teórico. Si > $200, debes recontar e investigar antes de que Mónica firme', c:'Mónica', d:'Cocina',
          correcta:'b', explicacion:'El arqueo es directamente tu responsabilidad. Mónica solo valida lo que tú reportaste.' },
        { pregunta:'Saltos en folios consecutivos sin explicación pueden indicar:',
          a:'Cosa normal', b:'Posible FRAUDE — tickets emitidos no reportados o anulados sin documentar. Cada folio faltante debe explicarse', c:'Error del POS sin importancia', d:'Problema técnico menor',
          correcta:'b', explicacion:'Es la bandera más sospechosa de fraude. Documenta y escala cualquier salto sin explicación.' },
        { pregunta:'Cuando Mónica viene a firmar, ¿cuál es la mejor postura profesional?',
          a:'Decir "todo limpio" aunque haya algo', b:'Si hay banderas: "Hay banderas, ya las explico" — reporta con honestidad y muestra que las viste y manejaste', c:'No decir nada', d:'Pasarle la responsabilidad',
        ...
        { pregunta:'Si las banderas en el Resumen escapan a tu control inmediato (ej: cortesía sin autorización del host), ¿qué haces antes de que Mónica firme?',
          a:'Esperas que firme', b:'Documentas en observaciones, avisas a Mónica del problema, esperas su decisión antes de cerrar', c:'Firmas tú', d:'Lo borras',
        ...
        { pregunta:'¿Cuál es tu mayor protección profesional al cierre?',
          a:'Firmar rápido', b:'Documentar lo que viste, comunicar claramente, no firmar "limpio" si no lo está. Honestidad > apariencia de perfección', c:'Esperar', d:'Echar la culpa a otros',
    ...
    // ---------- Módulo 7: Situaciones difíciles ----------
    {
      ...
      quiz: [
        { pregunta:'Un cliente molesto te reclama por la cuenta. ¿Qué haces?',
          a:'Le bajas el precio para que se calme', b:'Escuchas, ofreces llamar al gerente: "Permíteme, llamo a Gabriel para que platique con usted". NO autorizas descuentos por tu cuenta', c:'Discutes con él', d:'Le pides que se vaya',
        ...
        { pregunta:'Un cliente dice "soy amigo del dueño, dame descuento". ¿Qué haces?',
          a:'Le das descuento por si acaso', b:'NO autorizas. "Aprecio que sea cliente. La política es que solo el gerente autoriza descuentos. Permítame llamarlo". Llamas a Gabriel', c:'Lo ignoras', d:'Le cobras de más',
        ...
        { pregunta:'Si el POS falla y no procesa cobros, ¿qué haces?',
          a:'Cierras el restaurante', b:'Avisas a Mónica/Gabriel. Aceptas solo efectivo, anotas cada transacción en papel. Cuando vuelva el POS, capturas pendientes', c:'No haces nada', d:'Cobras al doble',
        ...
        { pregunta:'Sospechas que un billete es falso. ¿Cómo respondes?',
          a:'Acusas al cliente', b:'"Permítame un segundo a verificar" — llamas a Mónica/Gabriel para revisar al trasluz. NO acusas directamente al cliente', c:'Lo aceptas para no incomodar', d:'Le pides identificación',
    ...
    // ---------- Módulo 1: Tu rol como sub-gerente ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es la diferencia clave entre Gabriel (Gerente Restaurante) y tú (Encargado de Piso)?',
          a:'Ninguna, son lo mismo', b:'Gabriel es estratégico-operativo y autoriza cortesías; tú estás 100% en piso siempre y NO autorizas cortesías. Él es director técnico, tú eres capitán en cancha', c:'Tú eres su jefe', d:'Gabriel cocina, tú no',
          correcta:'b', explicacion:'Roles complementarios. Gabriel puede salir; tú estás siempre en piso. Tu valor está en presencia activa.' },
        { pregunta:'¿Qué decisiones financieras NO autorizas tú?',
          a:'Ninguna', b:'Cortesías, cancelaciones masivas, override de sellos, cambios de tarifa, descuentos. Solo Mónica y Gabriel manejan dinero', c:'Solo cortesías', d:'Decides todas',
          correcta:'b', explicacion:'Tu autoridad es operativa, no financiera. Llamas a Gabriel para decisiones de dinero.' },
        { pregunta:'¿Cuál es tu hábito principal durante un servicio?',
          a:'Estar en oficina pegado a la computadora', b:'Estar en piso, visible, atento. Caminar el salón cada 15-20 min, saludar clientes, preguntar a hosts si necesitan algo', c:'Esperar problemas', d:'Solo atender el teléfono',
    ...
    // ---------- Módulo 2: Los 4 tableros del sistema ----------
    {
      ...
      quiz: [
        { pregunta:'Si en el plano del salón ves una mesa Ocupada que lleva mucho más del promedio histórico (⏱), ¿qué haces?',
          a:'Nada', b:'Considera acción: averiguar si ya pidieron cuenta, si esperan postre, o si hay algún problema. Coordina con el host', c:'Echar al cliente', d:'Cancelar su mesa',
          correcta:'b', explicacion:'El promedio histórico te da señal de cuándo investigar. Coordinar con host para destrabar sin presionar al cliente.' },
        { pregunta:'En la bitácora ves una cortesía registrada con nombre de Mónica como "Autoriza" pero TÚ no la viste autorizar. ¿Qué haces?',
          a:'Lo dejas pasar', b:'Confirmas con Mónica directamente. Si dice sí → OK. Si dice no → posible fraude, escala', c:'La firmas tú', d:'La borras',
        ...
        { pregunta:'¿Con qué frecuencia debes revisar el plano del salón durante un servicio?',
          a:'Una vez al día', b:'Cada 10-15 minutos durante el servicio (entre rondas de piso)', c:'Solo al final', d:'Cuando un host te lo pide',
        ...
        { pregunta:'En charolas notas que cocina lleva 30 min sin registrar salidas, pero el buffet se ve activo. ¿Qué haces?',
          a:'Ignoras', b:'Vas a cocina y preguntas: "¿Están registrando? Lleva rato sin entrar nuevas". Recuerda capturar para que el cierre sea correcto', c:'Reportas a Germán', d:'Cancelas charolas',
          correcta:'b', explicacion:'Recordatorio amable, no regaño. La cajera necesita capturas correctas para el cierre. Tu rol es facilitarlo.' }
      ],
      minAprobatorio: 4
    },
    // ---------- Módulo 3: Resolución de bloqueos ----------
    {
      ...
      quiz: [
        { pregunta:'Llega un grupo de 12 personas sin reserva y NO caben en las mesas disponibles. ¿Qué haces?',
          a:'Bloqueas mesas reservadas para acomodarlos', b:'Sé honesto: "Por hoy no podemos, les ofrezco reserva para mañana o algunos en barra". Si dudas, llama a Gabriel', c:'Los echas con malos modos', d:'Aceptas y los acomodas mal',
        ...
        { pregunta:'Un cliente reclama que su comida estaba fría. ¿Cómo respondes?',
          a:'Le dices que está exagerando', b:'Escuchas, te disculpas, ofreces reposición sin cobro. Si fue grave, llamas a Gabriel para que vaya a la mesa', c:'Le cobras igual', d:'Lo regañas',
        ...
        { pregunta:'Hay conflicto entre el host y cocina visible en operación. ¿Qué haces?',
          a:'Tomas partido públicamente', b:'Apartas a las personas del cliente, escuchas brevemente, decides si es operativo. Después del servicio, conversas con ambos por separado', c:'Los regañas en público', d:'Los ignoras',
        ...
        { pregunta:'Si descubres que un cliente se fue sin pagar, ¿qué haces?',
          a:'Lo persigues físicamente', b:'NO lo persigues (riesgo). Verificas con cajera, documentas, avisas a Gabriel inmediato. Cancelación de ticket requiere autorización gerencial', c:'Le cobras a la cajera', d:'No haces nada',
        ...
        { pregunta:'¿En qué casos SIEMPRE llamas a Gabriel sin intentar resolver tú?',
          a:'En cualquier cosa', b:'Cliente VIP con queja seria, pérdida material grande, reclamo legal/sanitario, fraude sospechado, cliente que se fue sin pagar, falla del sistema mayor', c:'Solo si tienes ganas', d:'Nunca',
    ...
    // ---------- Módulo 4: Coordinación con el equipo ----------
    {
      ...
      quiz: [
        { pregunta:'Una host comete un error de captura mientras atiende a un cliente. ¿Qué haces?',
          a:'La regañas enfrente del cliente', b:'Esperas a que termine la interacción, luego le enseñas amable. Capacitar en tiempo real sin avergonzar', c:'Tomas su celular y lo haces tú', d:'La despides',
        ...
        { pregunta:'¿Operas la caja directamente cuando la cajera está abrumada?',
          a:'Sí, ayudo en todo', b:'NO. Separación de funciones. Mandas un host a apoyar tareas que no toquen caja, pero tú no operas dinero', c:'Solo en domingo', d:'Sí, todos los días',
        ...
        { pregunta:'¿Cuándo escalas DIRECTO a Mónica sin pasar por Gabriel?',
          a:'Cualquier cosa', b:'Cuando Gabriel no está disponible y la decisión no puede esperar, o cuando el asunto es gerencial-financiero, o cuando Gabriel mismo es parte del problema. SIEMPRE avisas a Gabriel después', c:'Nunca', d:'Cuando estoy enojado',
        ...
        { pregunta:'¿Cuál es el mejor signo de que estás haciendo bien tu rol como sub-gerente?',
          a:'Que todos te aplauden', b:'Las cosas funcionan cuando estás, pero no se nota. El equipo trabaja tranquilo, los clientes no notan tu rol pero todo fluye', c:'Que peleas mucho', d:'Que regañas seguido',
    ...
    // ---------- Módulo 5: Cierre del servicio ----------
    {
      ...
      quiz: [
        { pregunta:'30-45 min antes del cierre del servicio, ¿qué revisas en la bitácora?',
          a:'Solo el total', b:'Filas sin hora_sal, reservas pendientes en "En espera" que ya no llegarán, capturas raras (sin nombre, cortesías sin autoriza)', c:'Solo cortesías', d:'Nada',
        ...
        { pregunta:'Durante el cierre profundo de la cajera, ¿cuál es tu rol?',
          a:'Operar la caja', b:'Estar disponible para dudas, NO operar caja, ayudar a diagnosticar banderas operativas (cortesías), llamar a Gabriel si son financieras', c:'Capturar números por ella', d:'Apurarla',
        ...
        { pregunta:'Si Gabriel no estará al cierre y Mónica te confía coordinar, ¿qué haces?',
          a:'Tomas todas las decisiones', b:'Facilitas el cierre, reportas a Mónica por WhatsApp lo que pasa, si necesita firmar de gerente lo coordina remoto. SIEMPRE bajo su dirección', c:'Cierras solo', d:'Lo pospones',
        ...
        { pregunta:'Surge una bandera crítica al cierre (cortesía con nombre de Mónica que no la viste autorizar). ¿Qué haces?',
          a:'Dejas que la cajera firme', b:'Documentas evidencia (captura), llamas a Mónica inmediato, NO permites que cajera firme hasta que esté explicada', c:'La firmas tú', d:'La borras',
    ...
    // ---------- Módulo 2: El recetario ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es la diferencia entre el tab "Ingredientes" y el tab "Recetas"?',
          a:'Son lo mismo', b:'Ingredientes = catálogo maestro de insumos (tomate, sal). Recetas = combinación de ingredientes con cantidad, instrucciones y costo calculado', c:'Ingredientes son las recetas', d:'No hay diferencia',
          correcta:'b', explicacion:'Catálogo de insumos vs catálogo de recetas. Una receta usa varios ingredientes (y a veces sub-recetas).' },
        { pregunta:'Si una receta tuya tiene la bandera 📍 (costo con estimados), ¿qué significa?',
          a:'Que está mal', b:'Que al menos uno de sus ingredientes tiene precio estimado (no validado con factura). Weslley está validando los 297 estimados pendientes', c:'Que se debe borrar', d:'Que se cobra estimado',
        ...
        { pregunta:'Si necesitas usar un ingrediente nuevo (añades un platillo), ¿qué haces?',
          a:'Lo das de alta tú', b:'Avisas a Weslley (Comprador) para que él lo dé de alta con su precio. TÚ no editas el catálogo de ingredientes', c:'Lo escribes a mano en la receta', d:'Lo ignoras',
          correcta:'b', explicacion:'Separación de funciones: ingredientes los administra Weslley/Mónica. Tú propones cambios a recetas, no editas el catálogo de insumos.' },
        { pregunta:'En el tab Rentabilidad notas que una receta tuya pasó de costar $40 a $90 por porción. ¿Qué haces?',
          a:'Ignoras', b:'Investigas: probablemente un ingrediente importante subió de precio. Avisas a Weslley para confirmar; considera si la receta se puede simplificar', c:'Subes el precio del menú', d:'Quitas la receta',
    ...
    // ---------- Módulo 3: Editar y proponer cambios al recetario ----------
    {
      ...
      quiz: [
        { pregunta:'En Modelo B, ¿qué pasa cuando editas una receta?',
          a:'Se aplica directo al recetario', b:'Queda como propuesta pendiente. Mónica/Gabriel revisan y, si aprueban, aplican el cambio directo. Si rechazan, te avisan por qué', c:'Se borra la receta', d:'Tienes que esperar 24 horas',
        ...
        { pregunta:'Si modificas las líneas (ingredientes y cantidades) de una receta, ¿qué pasa con las líneas anteriores?',
          a:'Se conservan junto a las nuevas', b:'El sistema hace REEMPLAZO TOTAL: las líneas anteriores se sustituyen por la lista nueva completa. Por eso debes verificar que la lista completa esté bien antes de guardar', c:'No pasa nada', d:'Se duplican',
          correcta:'b', explicacion:'Reemplazo total — no es edición línea por línea. Verifica la lista completa antes de guardar.' },
        { pregunta:'En el modal de la receta, presionas el botón 📷 desde tu celular. ¿Qué pasa?',
          a:'Se cierra el sistema', b:'Te abre la cámara directo. Tomas foto del platillo terminado y se sube a la carpeta Drive "Fogueira · Fotos Recetario"', c:'Imprime la receta', d:'Llama a Mónica',
        ...
        { pregunta:'Quieres crear una receta nueva pero solo tienes nombre, área y 2 ingredientes — todavía no sabes las instrucciones. ¿Puedes guardarla?',
          a:'No, instrucciones son obligatorias', b:'Sí. Mínimo necesitas nombre, área y al menos 1 línea (ingrediente). Las instrucciones puedes completarlas después editando la receta', c:'No, debe estar todo', d:'Solo si Mónica te lo dice',
          correcta:'b', explicacion:'Mínimo viable: nombre, área, 1 línea. Instrucciones se completan después editando la propuesta o ya aprobada.' },
        { pregunta:'¿Cuál es uno de los esqueletos pendientes que Mónica te pidió resolver?',
          a:'Pizza margarita', b:'Chimichurri, Mantequilla crotones, Brazo de reina (3 esqueletos sin líneas)', c:'No hay pendientes', d:'Hamburguesa',
    ...
    // ---------- Módulo 4: Charolas y mermas en vivo ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cada cuándo capturas una charola en el sistema?',
          a:'Una vez al final del servicio', b:'Cada vez que sacas una charola al buffet (al momento o máximo 5 min después). Si esperas al final, pierdes precisión y se te olvidan charolas', c:'Solo cuando se acaba', d:'Una vez a la semana',
          correcta:'b', explicacion:'En vivo es la regla. Cada charola = un registro inmediato. Acumular pierde precisión.' },
        { pregunta:'Si una charola se cayó al piso o sobró del buffet, ¿qué haces?',
          a:'No la registras', b:'La capturas como merma con tipo (daño / sobrante) y motivo. Sin captura, parece "comida perdida sin justificación" y eso es bandera para la dirección', c:'La cobras al cliente', d:'La metes al refrigerador',
        ...
        { pregunta:'Sacaste una charola de papas que rinde aproximadamente 20 porciones. ¿Qué capturas en cantidad?',
          a:'El peso de la charola en kilos', b:'20 (porciones estimadas). El sistema cuenta porciones, no kilos', c:'1 (una charola)', d:'No capturas cantidad',
        ...
        { pregunta:'¿Por qué es recomendable vincular la charola a una receta del recetario?',
          a:'Es decorativo', b:'Permite al sistema calcular costo de la charola automáticamente (porciones × costo por porción) y descontar inventario si aplica', c:'Para presumir', d:'No sirve',
    ...
    // ---------- Módulo 5: Sistema completo de mermas ----------
    {
      ...
      quiz: [
        ...
        { pregunta:'Las recetas del sistema usan peso bruto del proveedor. ¿Qué beneficio tiene eso para la merma natural?',
          a:'Ninguno', b:'El peso bruto ya incluye la pérdida por cocción. Al descontar 300g de inventario por una charola, el sistema ya contempla los gramos que se pierden al cocinarse — no tienes que registrar esa pérdida por separado', c:'Que se cobra más al cliente', d:'Que el proveedor lo exige',
        ...
        { pregunta:'Sobró media charola de puré de papa al cierre del servicio. ¿Qué registras?',
          a:'Nada, ya estaba servida en el buffet', b:'Merma con causa "Sobrante al cierre del turno", cantidad 0.5. La fracción es válida y la causa cierra el inventario', c:'Una charola completa de merma', d:'Lo registra Mónica al día siguiente',
          correcta:'b', explicacion:'Merma parcial (0.5) con causa específica. Registrar el sobrante al cierre es parte del checklist obligatorio.' },
        { pregunta:'Al final del día el sistema muestra −13 unidades de diferencia en el inventario sin explicación. ¿Qué causó eso probablemente?',
          a:'Error del sistema', b:'No se registraron las mermas del día. Sin registro, el inventario descuadra y aparece como bandera roja para la dirección', c:'El proveedor entregó de menos', d:'Marcos lo capturó por error',
        ...
        { pregunta:'¿Puedes registrar una merma sin vincularla a una receta específica?',
          a:'No, la receta es obligatoria', b:'Sí. El sistema guarda el evento igualmente. Sin receta no calcula qué ingredientes se perdieron, pero es mejor merma sin receta que dejar el hueco sin documentar', c:'Solo Mónica puede registrar mermas sin receta', d:'No existe esa opción en el sistema',
    ...
    // ---------- Módulo 6: Cierre del servicio ----------
    {
      ...
      quiz: [
        { pregunta:'Estás 30 min antes del cierre y notas que olvidaste capturar 2 charolas que sacaste a las 2pm. ¿Qué haces?',
          a:'Las dejas pasar', b:'Las capturas ahora con observación "captura tardía" o las vinculas a la hora correspondiente. Mejor tarde que nunca', c:'Le dices a Marcos que las capture', d:'Te vas',
        ...
        { pregunta:'Vas a firmar tu sello de cierre y descubres que un cocinero capturó 50 porciones en lugar de 5 (dedazo). ¿Qué haces?',
          a:'Firmas igual', b:'NO firmas hasta corregirlo. Si puedes editar el registro mismo día, lo corriges. Si no, avisas a Mónica antes de firmar', c:'Borras la bitácora', d:'Lo dejas pasar',
          correcta:'b', explicacion:'Sello con bandera roja sin explicar = problema mayor después. Mejor tardar 1 hora más.' },
        { pregunta:'¿Tu sello de cierre lo puede firmar otra persona desde su cuenta?',
          a:'Sí, cualquiera', b:'No. Cada sello es autenticado por user_id. Solo tú firmas desde tu sesión. Si no estás, Mónica puede hacer override admin con motivo registrado', c:'Sí, Marcos', d:'Solo Germán',
        ...
        { pregunta:'Sobró comida del buffet al cierre del servicio. ¿Qué haces?',
          a:'No registras nada', b:'Capturas como merma sobrante con motivo "sobrante final servicio". Coordina con Marcos si fue compartido', c:'Te lo llevas', d:'Lo tiras sin más',
    ...
    // ---------- Módulo 1: Tu rol como churrasquero ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es la responsabilidad principal de Churrasca en Fogueira?',
          a:'Cobrar a los clientes', b:'Atender el rodizio: pasar carnes en espada por las mesas y registrar las salidas. También llevas el inventario semanal de tu zona', c:'Conciliar caja', d:'Hacer reservas',
          correcta:'b', explicacion:'Rodizio + inventario semanal de proteína. Tu zona es 100% carnes y parrilla.' },
        { pregunta:'¿Cuál es la diferencia entre tu zona y la de Sergio (Cocina)?',
          a:'Ninguna', b:'Sergio maneja cocina caliente/fría (salsas, ensaladas, guarniciones, postres). Tú manejas la carne en espada y parrilla. Coordinan el ritmo del buffet juntos', c:'Tú eres su jefe', d:'Sergio es chef y yo no',
        ...
        { pregunta:'¿Cuál pantalla del sistema es la MÁS importante para ti?',
          a:'Solo Charolas', b:'Inventario Churrasca semanal — es la pantalla central de tu rol porque ahí controlas lo que entra y sale de proteína día por día', c:'Reservaciones', d:'Conciliación',
        ...
        { pregunta:'¿Tu sello de cierre lo puede firmar Sergio (Cocina) por ti?',
          a:'Sí', b:'No. Cada sello es autenticado por user_id. Solo tú firmas desde tu sesión. Si no estás, Mónica puede hacer override admin con motivo', c:'Sí, son del mismo equipo', d:'Solo si Gabriel lo dice',
    ...
    // ---------- Módulo 2: Inventario Churrasca semanal ----------
    {
      ...
      quiz: [
        { pregunta:'En el inventario churrasca, ¿qué capturas como "Salida"?',
          a:'Lo que sale al buffet en espada', b:'Lo que dejas de tener crudo (sale del refrigerador al área de parrilla). Las espadas que pasan por las mesas se capturan en Charolas, no aquí', c:'Lo que cobra cajera', d:'No hay salida',
        ...
        { pregunta:'Si recibes una entrega del proveedor de 5 kg de picaña, ¿dónde la capturas?',
          a:'En Charolas', b:'En Inventario Churrasca como "Entrada" del día con la cantidad y opcionalmente notas (lote, proveedor)', c:'En Recetas', d:'No se captura',
        ...
        { pregunta:'¿Cuál es la fórmula automática que el sistema usa para calcular el "Final" del día?',
          a:'Inicial × Entrada', b:'Final = Inicial + Entrada - Salida - Merma. Pero puedes capturar Final manual si tu conteo físico no cuadra con la fórmula', c:'Inicial + Total', d:'No hay fórmula',
        ...
        { pregunta:'Una pieza de picaña te llegó con mal aspecto y la tiraste antes de cocinarla. ¿Cómo se captura?',
          a:'No se captura', b:'Como Merma del día con motivo "pieza con mal aspecto descartada antes de cocinar"', c:'Como Salida normal', d:'Se cobra al proveedor',
        ...
        { pregunta:'Al cierre del día, tu conteo físico del refri NO cuadra con el "Final" calculado por fórmula. ¿Qué haces?',
          a:'Lo dejas como está', b:'Capturas Final manual con observación explicando la diferencia. Una diferencia puede ser captura faltante, error previo o algo a investigar', c:'Borras todo', d:'Llamas al proveedor',
    ...
    // ---------- Módulo 3: Recetas de churrasca ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué información ADICIONAL es muy importante incluir en las instrucciones de una receta de churrasca?',
          a:'Solo el nombre', b:'Temperatura de parrilla, tiempo por lado, punto deseado y reposo después. Le sirve a tu ayudante para replicar sin ti', c:'Solo precio', d:'Solo foto',
          correcta:'b', explicacion:'Tiempos y temperatura son críticos en parrilla. Sin documentar, otros no pueden replicar.' },
        { pregunta:'Una receta nueva de costilla. ¿Qué % de merma capturas aproximadamente?',
          a:'5%', b:'~35-40% (la costilla pierde ~35% al cocinarse por grasa y líquidos)', c:'0%', d:'80%',
        ...
        { pregunta:'En el sistema, ¿qué pasa cuando editas una receta de churrasca?',
          a:'Se aplica directo', b:'Queda como propuesta pendiente. Mónica/Gabriel revisan y aprueban → cambio aplicado. Modelo B (igual que cocina)', c:'Se borra la receta', d:'Solo Sergio aprueba',
        ...
        { pregunta:'Una receta tiene área "ambas" (involucra cocina y churrasca). ¿Cómo se captura la charola al buffet?',
          a:'Sergio captura todo', b:'Tú capturas todo', c:'Coordinan: cada uno captura SU parte (la proteína tú, el acompañamiento Sergio). Sin coordinación pueden duplicar porciones', d:'No se captura',
    ...
    // ---------- Módulo 4: Charolas (espadas) en vivo ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es la diferencia entre merma en Inventario Churrasca y merma en Charolas?',
          a:'Son lo mismo', b:'Inventario = carne perdida ANTES de cocinarse (mal aspecto, contaminación). Charolas = espada YA COCINADA que no se sirvió completa (sobrante, se cayó, se quemó)', c:'Una es para Sergio', d:'Solo hay una merma',
        ...
        { pregunta:'Sacas una espada de picaña que rinde aproximadamente 12 porciones. ¿Qué capturas en cantidades?',
          a:'12 espadas', b:'Cantidad de espadas = 1, Porciones estimadas = 12. El sistema usa porciones para calcular costo', c:'Solo el peso', d:'No capturas cantidad',
        ...
        { pregunta:'¿Por qué es recomendable vincular cada charola a una receta del recetario?',
          a:'Es decorativo', b:'Permite al sistema calcular el costo de la espada automáticamente (porciones × costo por porción) e identificar el tipo de corte para análisis', c:'No sirve', d:'Para presumir',
        ...
        { pregunta:'Una espada de chorizo se cayó al piso y ya no se puede servir. ¿Dónde la registras?',
          a:'Inventario Churrasca', b:'Charolas Churrasca como merma tipo "daño" con motivo "espada cayó al piso después de cocinar"', c:'En ningún lado', d:'En recetas',
    ...
    // ---------- Módulo 5: El sistema completo de mermas en churrasca ----------
    {
      ...
      quiz: [
        { pregunta:'Una espada de picaña se cayó al piso después de salir de la parrilla. ¿Qué tipo de merma es y dónde la capturas?',
          a:'Merma de inventario en Inventario Churrasca', b:'Merma de charolas (post-cocción) en Charolas Churrasca: activo "Es merma", causa "Se cayó / accidente en cocina"', c:'No se captura', d:'Merma natural, ya está en la receta',
        ...
        { pregunta:'Una pieza de fraldinha cruda llegó con mal olor del proveedor y la descartaste. ¿Dónde la registras?',
          a:'En Charolas Churrasca como merma', b:'En Inventario Churrasca como Merma del día con observación "pieza con mal olor descartada antes de cocinar"', c:'No se registra', d:'En el recetario',
        ...
        { pregunta:'Al cierre del servicio sobró media espada de costilla que ya no se puede guardar. ¿Cuántas espadas capturas como merma de charola?',
          a:'1 espada entera', b:'0 (no se captura si es poca cantidad)', c:'0.5 espadas — el sistema acepta decimales para mermas parciales', d:'2 espadas',
        ...
        { pregunta:'¿Por qué la pérdida de peso de la carne al cocinarse (picaña pierde ~25-30%) NO se captura como merma explícita?',
          a:'Porque no importa', b:'Porque ya está incluida en el peso bruto de la receta. El sistema contempla ese porcentaje en el costeo. Solo capturas merma explícita cuando algo sale MAL (caída, quemado, sobrante)', c:'Porque es muy pequeña', d:'Porque la registra Sergio',
        ...
        { pregunta:'Terminaste el turno y el sistema muestra 1.5 kg de picaña sin explicar (inventario no cuadra). ¿Qué habría pasado si hubieras registrado tus mermas?',
          a:'El resultado sería igual', b:'Esa diferencia quedaría explicada por tus registros de merma, sin bandera roja. Sin registro, Mónica ve un hueco que parece pérdida oculta aunque sea legítima', c:'El sistema lo resuelve solo', d:'No importa el cuadre',
    ...
    // ---------- Módulo 6: Cierre del servicio ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es la diferencia clave de tu cierre vs el de Sergio (Cocina)?',
          a:'Ninguna', b:'Tú haces conteo físico del refri (Inventario Churrasca) y comparas con el Final calculado. Esa verificación física es central a tu rol porque la proteína es el insumo más caro', c:'Yo no firmo', d:'Sergio cuenta también',
        ...
        { pregunta:'En tu conteo físico al cierre, encuentras 1.5 kg menos de picaña de lo que dice el "Final" calculado. ¿Qué haces?',
          a:'Lo dejas así', b:'Capturas Final manual con la cantidad real (la del conteo físico) y agregas observación explicando: "Conteo físico difiere de fórmula en -1.5 kg, posible captura faltante o pérdida no documentada"', c:'Borras todo', d:'Cobras al proveedor',
          correcta:'b', explicacion:'Captura Final real + observación documentada. Mónica revisa y decide si investiga.' },
        { pregunta:'¿Tu sello de cierre lo puede firmar Sergio (Cocina) por ti?',
          a:'Sí', b:'No. Cada sello es autenticado por user_id. Solo tú firmas desde tu sesión. Si no estás, override admin de Mónica con motivo', c:'Solo el lunes', d:'Solo si lo dice Gabriel',
        ...
        { pregunta:'¿Qué es el "cierre semanal del inventario churrasca"?',
          a:'Lo mismo que el diario', b:'Adicional al diario: revisas la semana completa Lun-Dom, verificas capturas de cada día y comparas total con conteo físico. Mónica usa esto para calcular costo real de proteína y planear compra siguiente', c:'Solo cuando hay luna llena', d:'No existe',
    ...
    // ---------- Módulo 1: Tu rol como auditor interno ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es el principio fundamental de tu rol como auditor interno?',
          a:'Operar todas las áreas', b:'NO operas lo que auditas. Tu acceso es de lectura (con excepciones puntuales). Tu trabajo es revisar, identificar anomalías, documentar hallazgos y reportar', c:'Cobrar', d:'Cocinar',
          correcta:'b', explicacion:'No-operación = principio core. Si tú operas y auditas hay conflicto de interés.' },
        { pregunta:'¿Qué es la "separación de funciones" en Fogueira?',
          a:'Que cada quien trabaje en su área física', b:'Que una sola persona NO tenga control total de un proceso. Ej: Chef propone receta, Mónica aprueba; cajera cobra, Mónica/Gabriel autorizan cortesías; etc.', c:'Que se separen los turnos', d:'Solo una regla legal',
          correcta:'b', explicacion:'Control clave: dividir responsabilidades evita fraude y errores no detectados.' },
        { pregunta:'Descubres que la cajera firmó el cierre sin que Mónica/Gabriel revisaran las cortesías del día. ¿Qué tipo de hallazgo es?',
          a:'No es hallazgo', b:'Hallazgo de control: violación de separación de funciones. Una sola persona controló el proceso. Documentas con evidencia y reportas', c:'Solo error operativo', d:'No es tu problema',
          correcta:'b', explicacion:'Violación de separación = hallazgo serio. Documentas y escalas.' },
        { pregunta:'En el sistema, ¿puedes editar una conciliación cerrada?',
          a:'Sí, todas', b:'No. Tu rol es read-only. La conciliación cerrada solo se modifica con override admin de gerencia con motivo registrado en auditoría', c:'Solo lunes', d:'Solo si tú la firmaste',
    ...
    // ---------- Módulo 2: Banderas rojas — dónde buscar ----------
    {
      ...
      quiz: [
        { pregunta:'Encuentras una cortesía en bitácora sin nombre del autorizante. ¿Qué tipo de hallazgo es?',
          a:'Sin importancia', b:'Bandera roja: falta de control. Solo Mónica/Gabriel autorizan cortesías. Sin firma no hay trazabilidad → puede ser cortesía no autorizada o error de captura. Documentas y investigas', c:'Por culpa del cliente', d:'Sin clasificar',
          correcta:'b', explicacion:'Cortesía sin autorización = control violado. Investigación obligatoria.' },
        { pregunta:'Los folios del POS muestran saltos (101, 102, 105). ¿Qué significa típicamente?',
          a:'Numeración aleatoria', b:'Tickets cancelados. Cancelaciones requieren autorización gerencial. Saltos sin documentar = cancelaciones sin trazabilidad. Investigas cada salto', c:'No significa nada', d:'Que se acabaron',
          correcta:'b', explicacion:'Saltos en folios = tickets cancelados. Sin autorización registrada = bandera roja.' },
        { pregunta:'¿Cuál es la actitud INCORRECTA frente a una bandera roja?',
          a:'Investigar con calma', b:'Ignorarla porque "siempre pasan", asumir fraude sin investigar, confrontar al equipo sin evidencia, o acumularlas sin reportar', c:'Documentar', d:'Hablar con la persona involucrada',
        ...
        { pregunta:'En la hoja Sellos ves un sello con es_override = true pero motivo_override vacío. ¿Qué haces?',
          a:'Lo dejas pasar', b:'Es bandera: override sin motivo viola el control. Hablas con el admin que firmó, documentas, escalas a Mónica si no hay justificación clara', c:'Lo borras', d:'Felicitas al admin',
          correcta:'b', explicacion:'Override sin motivo = excepción no documentada. Investigación obligatoria.' },
        { pregunta:'En el inventario churrasca de la semana, las diferencias entre fórmula y conteo físico son grandes y recurrentes (todos los días). ¿Qué hallazgo registras?',
          a:'Sin importancia', b:'Hallazgo de control: pérdida de proteína sin documentar. Patrón repetitivo descarta error puntual. Reportas a Mónica con evidencia (capturas de la semana)', c:'Le echas la culpa al proveedor', d:'No es tu problema',
    ...
    // ---------- Módulo 3: Hallazgos y papeles de trabajo ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es la diferencia entre una bandera roja y un hallazgo?',
          a:'Son lo mismo', b:'Bandera roja = señal automática del sistema. Hallazgo = conclusión documentada después de investigar la bandera. Una bandera puede resolverse en error de captura (no es hallazgo) o confirmarse como falla de control (sí es hallazgo)', c:'La bandera es para Mónica', d:'No hay diferencia',
          correcta:'b', explicacion:'Bandera = automática. Hallazgo = tu conclusión investigada. No toda bandera se convierte en hallazgo.' },
        { pregunta:'¿Cuáles son los 5 elementos de un hallazgo bien documentado?',
          a:'Solo descripción y opinión', b:'Condición (qué pasó), Criterio (qué debió pasar), Causa (por qué pasó), Efecto (impacto), Recomendación (qué proponer)', c:'Solo evidencia', d:'Solo recomendación',
          correcta:'b', explicacion:'Estructura clásica de hallazgo de auditoría: 5 elementos en orden lógico.' },
        { pregunta:'Encuentras un faltante en arqueo de caja de $7,000 sin explicación. ¿Qué severidad asignas?',
          a:'Información', b:'Hallazgo crítico: faltante material >$1,000 sin explicación es riesgo financiero significativo. Reporte inmediato a dirección', c:'Observación menor', d:'Sin importancia',
        ...
        { pregunta:'¿Qué es un "papel de trabajo" en auditoría?',
          a:'Cualquier documento', b:'Documento donde organizas tus hallazgos del período revisado, con plantilla mínima: condición/criterio/causa/efecto/recomendación + evidencia. Reporta hallazgos por severidad', c:'Solo un nombre técnico', d:'No existe',
          correcta:'b', explicacion:'Papel de trabajo = entregable de tu revisión. Estructura clara, evidencia respaldada.' },
        { pregunta:'¿Qué NO es tu rol como auditor interno?',
          a:'Documentar y reportar', b:'Ser policía que confronta al equipo, ser acusador con juicios, ser operador que resuelve el problema, ser gerente que toma decisiones. Tu rol es revisar, documentar, recomendar', c:'Identificar banderas', d:'Investigar',
    ...
    // ---------- Módulo 4: Verificación de identidad y soft-delete ----------
    {
      ...
      quiz: [
        { pregunta:'¿Por qué el sistema guarda "snapshot" del nombre, email y rol al momento del sello (no referencia)?',
          a:'Porque es más fácil', b:'Porque si después el usuario cambia de rol o nombre, el sello sigue diciendo lo que firmó EN SU MOMENTO. Esto es crítico para auditoría histórica', c:'No hay razón', d:'Por seguridad informática',
          correcta:'b', explicacion:'Snapshot preserva integridad histórica. Lo que firmó en su momento queda registrado independientemente de cambios posteriores.' },
        { pregunta:'En BitacoraFilas, una fila tiene `borrada_at` llena con fecha. ¿Qué significa?',
          a:'Que se eliminó físicamente', b:'Soft-delete: la fila sigue accesible para auditoría con todos sus datos. Solo se marcó como borrada (UI operativa NO la muestra, pero tú sí la ves desde auditoría)', c:'Que se duplicó', d:'Error del sistema',
        ...
        { pregunta:'¿Cómo verificas la autenticidad fuerte de un sello "Mónica · 23:15"?',
          a:'Solo viendo el campo user_email', b:'Por el conjunto de evidencia: bitácora de sesiones (login en esa hora), comunicación (Mónica reportó estar), IP, conversación directa con ella. Un solo campo no es prueba fuerte', c:'No se puede verificar', d:'Llamando al cliente',
        ...
        { pregunta:'¿Qué tipo de patrón con overrides admin es bandera?',
          a:'Cualquier override', b:'Override sin motivo o con motivo genérico ("override"), múltiples overrides del mismo usuario en corto tiempo, override en horario inusual', c:'Solo si es de fin de semana', d:'No hay patrones',
    ...
    // ---------- Módulo 5: Reportes y herramientas ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es tu herramienta principal para análisis multi-fecha?',
          a:'Conciliación del día', b:'Histórico de Conciliaciones — permite filtrar rangos, ver KPIs, exportar CSV y abrir cualquier día específico (read-only)', c:'Solo el sheet', d:'No hay herramienta',
          correcta:'b', explicacion:'Histórico es vista PRO con filtros y export. Tu base para revisión semanal/mensual.' },
        { pregunta:'En el panel de Certificaciones, detectas que un usuario fue reseteado 4 veces en 2 meses. ¿Qué significa típicamente?',
          a:'Nada raro', b:'Posible bandera: el reset puede estar usándose como atajo en lugar de capacitación real. Investigas quién resetea, por qué, y si hay capacitación real entre intentos', c:'Que el usuario es muy estudioso', d:'Error del sistema',
        ...
        { pregunta:'¿Cuál es la regla de oro al acceder al Google Sheet base directamente?',
          a:'Modifica todo lo que quieras', b:'Solo lees. NO escribes. Si modificas, rompes la integridad del sistema. Tu rol es de lectura', c:'Solo escribes los lunes', d:'Pides permiso a Sergio',
        ...
        { pregunta:'¿Cuál es la frecuencia recomendada para producir tu papel de trabajo formal?',
          a:'Diaria', b:'Mensual (primeros 5 días del mes siguiente), con revisión semanal de inventario churrasca y reporte a dirección mensual', c:'Anual', d:'Solo cuando alguien lo pide',
    ...
    // ---------- Módulo 1: Tu rol como comprador ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es la responsabilidad principal del rol Comprador?',
          a:'Cocinar', b:'Validar y mantener actualizados los precios de los ingredientes, negociar con proveedores, validar los 297 precios estimados (📍), reportar variaciones', c:'Operar caja', d:'Hacer reservas',
          correcta:'b', explicacion:'Tu rol es 100% sobre precios y proveedores. Sin ti, el costeo de recetas se vuelve impreciso.' },
        { pregunta:'¿Por qué es estratégico que tú actualices precios a tiempo?',
          a:'No importa', b:'Si el aceite sube 38% y no actualizas, el costo del platillo aumenta sin reflejarse en el menú. Mónica no puede tomar decisiones (ajustar menú, cambiar proveedor, simplificar receta) si los precios están desactualizados', c:'Solo por orden', d:'Para presumir',
        ...
        { pregunta:'¿Tú editas recetas (cantidades, ingredientes, instrucciones)?',
          a:'Sí, todas', b:'No. Tu rol edita PRECIOS de ingredientes. Las recetas las modifican Sergio (Cocina) o Marcos (Churrasca) con Modelo B (proponen → Mónica aprueba). Tú las ves en lectura para entender qué se afecta cuando cambias un precio', c:'Solo los lunes', d:'Solo si Mónica te lo pide',
          correcta:'b', explicacion:'Separación de funciones: precios eres tú; recetas son los chefs. Mantenes el catálogo de insumos, no las recetas.' },
        { pregunta:'¿Cuál es la pantalla del sistema que MÁS vas a usar?',
          a:'Conciliación', b:'Recetas y costeo, especialmente el tab Ingredientes (donde editas precios directo)', c:'Bitácora', d:'Charolas',
    ...
    // ---------- Módulo 2: Tab Ingredientes — edición inline ----------
    {
      ...
      quiz: [
        { pregunta:'¿Tu edición de precio en el tab Ingredientes es inmediata o es propuesta?',
          a:'Es propuesta como Sergio/Marcos', b:'Es INMEDIATA. A diferencia de Sergio/Marcos (Modelo B), tú editas precios directo. El cambio se propaga al instante a todas las recetas que usan ese ingrediente', c:'Solo en lunes', d:'Mónica aprueba',
          correcta:'b', explicacion:'Inmediato = tu poder operativo. También significa que tu error se propaga al instante: verifica antes de capturar.' },
        { pregunta:'Qué pasa con las recetas cuando cambias el precio de un ingrediente?',
          a:'Nada', b:'Todas las recetas que usan ese ingrediente recalculan automáticamente su costo. El tab Rentabilidad refleja el cambio', c:'Solo las nuevas', d:'Hay que recalcular a mano',
        ...
        { pregunta:'¿Cuándo quitas el flag 📍 (estimado) de un ingrediente?',
          a:'Nunca', b:'Cuando ya tienes el precio confirmado por factura o proveedor real (lo dejas de "estimado"). Esto le dice al sistema que el costo de ese ingrediente ya es preciso, no aproximado', c:'Cada lunes', d:'Solo si Mónica lo pide',
        ...
        { pregunta:'Sergio te dice que necesita "queso provoleta" que no existe en el catálogo. ¿Qué haces?',
          a:'No es tu rol', b:'En el tab Ingredientes, botón "+ Nuevo ingrediente", llenas nombre, categoría, unidad y precio (estimado si no lo sabes con factura todavía). Queda disponible para que Sergio lo use en su receta', c:'Le pides a Mónica que lo cree', d:'Le dices que use otro',
          correcta:'b', explicacion:'Crear ingredientes nuevos es tu rol. Sergio propone usar el insumo en una receta, pero tú lo das de alta en el catálogo.' },
        { pregunta:'Capturas un precio y al momento te das cuenta que pusiste $1,800 cuando era $180 (un cero de más). ¿Qué pasa?',
          a:'Nada grave', b:'El error se propaga inmediato a todas las recetas que usan ese ingrediente. Las verás 10× más caras en Rentabilidad. Lo corriges de inmediato (queda en auditoría) y revisas el tab Rentabilidad para confirmar que regresó a normal', c:'Se cancela solo', d:'No se puede corregir',
    ...
    // ---------- Módulo 3: Validar los 297 estimados ----------
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es el criterio MÁS importante para priorizar qué estimados validar primero?',
          a:'Orden alfabético', b:'Combinación de volumen de uso (recetas que lo usan, porciones diarias), precio relativo (caros mueven más) y volatilidad (precios que cambian seguido)', c:'Por color', d:'Aleatorio',
        ...
        { pregunta:'Validas un ingrediente y descubres que el precio real es 3× lo estimado ($150 vs $50). ¿Qué haces?',
          a:'Lo cambias en silencio', b:'Capturas el precio real, quitas flag 📍, y avisas a Mónica de inmediato porque las recetas que lo usaban estaban con costo muy subestimado y puede afectar decisiones de menú/recetas', c:'No lo cambias', d:'Espero al fin de mes',
          correcta:'b', explicacion:'Diferencia >30% es señal de mal estimado severo. Mónica necesita reaccionar (ajustar menú, reformular).' },
        { pregunta:'¿Cuál es el ritmo razonable para ir validando los 297 estimados?',
          a:'Todo en una semana', b:'10-20 por semana, en paralelo al día a día. En ~6 meses se completan todos. No es urgente sino constante', c:'No validarlos', d:'Solo cuando alguien lo pide',
        ...
        { pregunta:'¿Qué NO debes hacer al validar estimados?',
          a:'Verificar contra factura', b:'Validar al ojo sin factura, quitar el flag 📍 sin actualizar el precio, validar todos rápido sin verificar, no avisar a Mónica cuando un estimado era muy malo', c:'Capturar precio real', d:'Avisar a Mónica',
    ...
    // ---------- Módulo 4: Reporte semanal de variaciones ----------
    {
      ...
      quiz: [
        { pregunta:'¿Por qué Mónica necesita un reporte semanal tuyo en vez de revisar el sistema directamente?',
          a:'No tiene acceso', b:'Sin tu reporte tendría que revisar 800 ingredientes para detectar cambios — no es práctico. Tu reporte es la visión consolidada de "qué está pasando con costos esta semana"', c:'Por reglas legales', d:'Para presumir',
        ...
        { pregunta:'¿Cuándo NO debes esperar al reporte semanal y avisar inmediato?',
          a:'Nunca', b:'Cuando hay un cambio grande (ej: subida >30% en un ingrediente importante o bajada equivalente). En ese caso WhatsApp directo a Mónica sin esperar al lunes', c:'Solo si Mónica pregunta', d:'Cada cambio individual',
        ...
        { pregunta:'¿Qué información NO debe faltar en cada subida/bajada notable del reporte?',
          a:'Solo el nombre', b:'Precio anterior, precio nuevo, variación % y recetas afectadas (al menos cuántas, idealmente cuáles). Idealmente también acción sugerida', c:'Solo el ingrediente', d:'Solo la fecha',
          correcta:'b', explicacion:'Sin esos datos, Mónica no puede tomar decisión. El reporte tiene que ser actuable.' },
        { pregunta:'¿Qué longitud razonable tiene el reporte semanal?',
          a:'10 páginas', b:'1-2 páginas. No es documento académico, es brief operativo. Lo importante es que destaque lo crítico al inicio (subidas grandes, alertas)', c:'Una sola línea', d:'No tiene límite',
    ...
    // ---------- Módulo 5: Importar SR12 ----------
    {
      ...
      quiz: [
        { pregunta:'¿Qué actualiza la importación SR12 en Fogueira?',
          a:'Las recetas completas', b:'El campo ultimo_costo de cada ingrediente que matcheó con el POS. Las recetas recalculan su costo automáticamente a partir de eso', c:'Solo los huérfanos', d:'Los precios del menú',
        ...
        { pregunta:'¿Qué es el dry-run y por qué es importante revisarlo?',
          a:'Es el log final', b:'Es un preview que muestra exactamente qué va a cambiar (ingrediente, costo actual vs SR12, variación %) ANTES de aplicar nada. Detectas divergencias raras antes de que se propaguen a recetas', c:'Es opcional', d:'Solo lo ve admin',
          correcta:'b', explicacion:'Dry-run = red de seguridad. No apliques sin revisarlo.' },
        { pregunta:'Un ingrediente en el dry-run aparece con variación de +180%. ¿Qué haces?',
          a:'Aplicas directo', b:'Lo investigas antes de aplicar: puede ser un cambio de precio real, error de captura en el POS, o diferencia de unidades (kg vs gr). Si no se explica, consúltale a IT o a Mónica', c:'Lo ignoras', d:'Lo eliminas del sistema',
          correcta:'b', explicacion:'Variación >50% es señal de alerta. Puede ser real o error. Verifica antes de propagar.' },
        { pregunta:'¿Qué son los "huérfanos" en la importación?',
          a:'Ingredientes eliminados', b:'Ingredientes del SR12 que el sistema no pudo ligar automáticamente a un ingrediente Fogueira. Puedes vincularlos manualmente — el vínculo queda guardado para importaciones futuras', c:'Errores del sistema', d:'Ingredientes nuevos creados',
        ...
        { pregunta:'¿Con qué frecuencia deberías importar SR12 en operación normal?',
          a:'Cada hora', b:'Quincenal o mensual en operación normal; semanal cuando hay volatilidad alta; inmediato después de negociaciones importantes; siempre antes de que Mónica revise rentabilidad del mes', c:'Una sola vez', d:'Solo admin puede decidirlo',
    ...
    // Módulo 6: Comprador profesional, rol estratégico y ética
    {
      ...
      quiz: [
        { pregunta:'¿Cuál es la diferencia clave entre un comprador transaccional y uno estratégico?',
          a:'El sueldo nada más', b:'El transaccional procesa pedidos según se le pidan; el estratégico anticipa, negocia, optimiza el costo total y mitiga riesgo. Su métrica es ahorro/OTIF/calidad/riesgo, no número de órdenes', c:'No hay diferencia', d:'El estratégico solo trabaja en lunes',
          correcta:'b', explicacion:'El estratégico genera valor; el transaccional ejecuta tareas. La empresa que confunde los dos sub-paga al rol estratégico.' },
        { pregunta:'Las 5 R de las compras son:',
          a:'Rápido, recio, rojo, raro, rico', b:'Right product, Right quality, Right quantity, Right time, Right price (producto correcto, calidad correcta, cantidad correcta, tiempo correcto y precio correcto)', c:'Solo el precio importa', d:'Solo aplica en exportación',
        ...
        { pregunta:'Un proveedor te ofrece una comisión personal del 2% por cada compra que le adjudiques. ¿Qué haces?',
          a:'Aceptas si nadie se entera', b:'Le respondes que no aceptas comisiones personales y le pides que aplique ese 2% como descuento en la factura. Reportas por escrito a Mónica el mismo día', c:'Lo aceptas si es chico', d:'Lo dejas pasar',
        ...
        { pregunta:'Tu primo tiene una distribuidora de carnes y quieres incluirlo entre las cotizaciones. ¿Cuál es la práctica correcta?',
          a:'No incluirlo nunca', b:'Declararlo POR ESCRITO a Mónica como conflicto de interés antes de iniciar la cotización, y pedir que ella tome la decisión final de adjudicación con base en las 3 cotizaciones objetivas', c:'Incluirlo y darle el contrato', d:'Mentir y decir que no es tu primo',
          correcta:'b', explicacion:'Declararlo por escrito te protege y permite competencia limpia. Si gana por mejor oferta, perfecto. Si pierde, también. Lo prohibido es ocultarlo.' },
        { pregunta:'¿Por qué el principio de "3 manos" (cotización / autorización / recepción) en cada compra?',
          a:'Es burocracia inútil', b:'Si una sola persona controla las 3 funciones tiene carta blanca para defraudar (pagar facturas inventadas, recibir menos de lo facturado, etc.). Separar funciones protege a la empresa Y al comprador (nadie puede acusarte si nunca tuviste acceso a pago/recepción)', c:'Es solo en empresas grandes', d:'Solo aplica si Mónica lo dice',
    ...
    // Módulo 7: La regla de las 3 cotizaciones
    {
      ...
      quiz: [
        { pregunta:'¿Por qué la regla es 3 cotizaciones específicamente y no 1, 2 o 5?',
          a:'Porque sí', b:'1 no da referencia (precio en el aire); 2 puede ser colusión; 5+ desperdicia tiempo (tuyo y del proveedor) y desincentiva que te coticen. 3 es el equilibrio entre rigor y costo de proceso, y es el estándar internacional (ISO 9001, COSO)', c:'Es decisión de Mónica', d:'Aplica solo en gobierno',
        ...
        { pregunta:'¿Cuál es una situación legítima donde NO se aplica la regla de 3 cotizaciones?',
          a:'Nunca hay excepciones', b:'Emergencias documentadas (urgencia operativa real), productos exclusivos de un solo proveedor, compras spot bajo umbral, acuerdos marco vigentes, servicios públicos regulados — todas con documentación escrita justificando la excepción', c:'Cuando el proveedor es amigo', d:'Cuando tienes prisa nada más',
        ...
        { pregunta:'Vas a cotizar suministro mensual de picaña. ¿Qué combinación de candidatos es más adecuada?',
          a:'3 carnicerías de barrio', b:'Mezcla: mínimo 1 proveedor establecido (incumbente) + 1 challenger nuevo + 1 más a tu juicio. Tamaño relativo coherente (los 3 deben poder atender tu volumen). Diversificación geográfica si es posible', c:'1 mega-distribuidor + 2 mini', d:'Los 3 que estén más cerca',
        ...
        { pregunta:'¿Qué NO debe faltar en un RFQ profesional?',
          a:'Solo el precio que quieres', b:'Especificación exacta del producto, cantidad y frecuencia, lugar y horario de entrega, condiciones de pago propuestas, plazo de respuesta (mín. 3 días hábiles), vigencia de la cotización, requisitos fiscales (CSF, 32D), tu contacto', c:'Una amenaza', d:'Solo el nombre del producto',
          correcta:'b', explicacion:'Sin especificación exacta y sin plazo, las 3 cotizaciones serán incomparables. Sin vigencia, el proveedor cambia el precio cuando le plazca.' },
        { pregunta:'Recibes las 3 cotizaciones y el más barato es Prov B con $185/kg. Pero su OTIF histórico es 75% y no ofrece garantía. ¿Cómo decides?',
          a:'Vas con B porque es el más barato', b:'Construyes un cuadro comparativo ponderado (precio 25%, calidad 25%, OTIF 15%, condiciones pago 10%, capacidad 10%, garantía 10%, fiscal 5%). Calculas puntaje. Documentas la decisión por escrito explicando por qué NO el más barato. Mónica autoriza', c:'Le pides la opinión a Sergio nada más', d:'Vas con el de mayor precio sin pensar',
    ...
    // Módulo 8: Evaluar proveedores - TCO, OTIF, riesgo
    {
      ...
      quiz: [
        { pregunta:'¿Qué es el TCO (Total Cost of Ownership) en compras?',
          a:'Solo el precio unitario', b:'Costo total real de comprarle a un proveedor: precio + costo de no-calidad + costo de retraso (compra pánico, horas extra) + costo de servicio (tiempo gestión problemas) + costo financiero (días de crédito)', c:'Un impuesto', d:'Algo solo para empresas grandes',
          correcta:'b', explicacion:'TCO captura lo que el precio unitario esconde. Un proveedor 5% más barato puede ser 10% más caro en TCO si falla, daña calidad o requiere mucha gestión.' },
        { pregunta:'¿Cuál es el target razonable de OTIF (On-Time In-Full) para un proveedor de Fogueira?',
          a:'Cualquiera está bien', b:'>90% (entrega a tiempo Y completa). Por debajo de 80%, el proveedor te está costando más en compras pánico y horas extra de lo que aparenta', c:'30%', d:'No se mide',
        ...
        { pregunta:'Prov A: $200/kg, OTIF 95%, calidad 5/5. Prov B: $185/kg, OTIF 75%, calidad 3/5. ¿Cuál es la decisión correcta?',
          a:'B porque es más barato', b:'Calculas TCO real (incluye costo pánico cuando B falla 25% del tiempo, no-calidad, gestión). En el ejemplo, A termina siendo $44,980/año MÁS BARATO en TCO que B. Decides A documentando el análisis', c:'Pides a Mónica que decida ella', d:'Le tiras un volado',
        ...
        { pregunta:'¿Por qué se recomienda no concentrar más del 70% del suministro de un insumo crítico en un solo proveedor?',
          a:'Por capricho', b:'Si concentras 100% y ese proveedor falla (huelga, sanitario, pelea contractual), te quedas sin producto de un día a otro. Dividir 70/30 con un backup activo da continuidad operativa. La pequeña pérdida en descuento por volumen es prima de seguro', c:'No es necesario', d:'Solo en empresas grandes',
        ...
        { pregunta:'¿Qué es la lista 69-B del SAT y por qué importa antes de dar de alta a un proveedor?',
          a:'Una lista de mejores proveedores', b:'Lista pública del SAT con empresas que han emitido facturas presuntamente falsas (EFOS — Empresas que Facturan Operaciones Simuladas). Si compras a alguien en lista 69-B definitiva, tus facturas con él pueden ser NO deducibles y puedes ser corresponsable. Hay que verificar SIEMPRE antes de alta', c:'No existe', d:'Es secreta',
    ...
    // Módulo 9: Negociación profesional
    {
      ...
      quiz: [
        { pregunta:'¿Qué es el BATNA y por qué es crítico antes de toda negociación?',
          a:'Una técnica de presión', b:'Best Alternative to Negotiated Agreement: tu plan B real si esta negociación no se cierra. Si tu BATNA es fuerte (2 alternativas decentes en mano), puedes negociar duro y caminarte. Si es débil, estás a merced del proveedor. La regla de 3 cotizaciones ES tu BATNA', c:'Un acrónimo legal', d:'Algo solo de empresas grandes',
          correcta:'b', explicacion:'Sin BATNA estás a merced del proveedor. Por eso las 3 cotizaciones no son burocracia: son tu poder de negociación.' },
        { pregunta:'Un proveedor te dice "es nuestro mejor precio, no podemos bajar". ¿Cuál es la mejor respuesta profesional?',
          a:'Romper la negociación', b:'"Entiendo. ¿Pueden compartir el desglose para que entendamos dónde está el costo? Tal vez podamos identificar áreas donde alguno tenga flexibilidad". Pides DESGLOSE, no descuento — abre conversación más productiva', c:'Aceptar nada más', d:'Insultarlo',
        ...
        { pregunta:'Además del precio, ¿qué palancas puedes negociar con un proveedor?',
          a:'Solo el precio importa', b:'Plazos de pago (15/30/45 días tiene costo financiero real), condiciones de entrega (frecuencia/horario), tamaño de lote mínimo, garantías y devoluciones, precio escalonado por volumen, compromiso de precio fijo (lock), exclusividad parcial, servicios adicionales', c:'Solo la fecha de entrega', d:'No hay otras palancas',
          correcta:'b', explicacion:'Una negociación profesional toca múltiples palancas. Bajar el precio 2% puede ser menos valioso que ganar 15 días extra de plazo o cláusula de garantía robusta.' },
        { pregunta:'Vas a comprar carne para los próximos 6 meses. ¿Acuerdo marco o spot?',
          a:'Spot siempre', b:'Acuerdo marco 6 meses con cláusula de revisión cada 3 meses (si el costo de mercado cambia >7% se renegocia). La carne es insumo crítico y relativamente estable; el marco te da predictibilidad de costo, descuento por compromiso de volumen, y la cláusula te protege de quedarte arriba del mercado', c:'Por el clima', d:'Lo que diga el proveedor',
    ...
    // Módulo 10: Fiscal, control interno y KPIs
    {
      ...
      quiz: [
        { pregunta:'Antes de dar de alta a un proveedor nuevo, ¿qué documentos fiscales SIEMPRE debes pedir y archivar?',
          a:'Solo el nombre y RFC', b:'Constancia de Situación Fiscal vigente (RFC, régimen, domicilio fiscal); Opinión de Cumplimiento positiva (32D); verificación de que NO está en lista 69-B SAT; carátula bancaria validada; datos de contacto. Renovar 32D cada 6 meses', c:'Solo una factura cualquiera', d:'No hace falta nada',
        ...
        { pregunta:'¿Qué es y cuándo debe emitirse el complemento de pagos?',
          a:'Es opcional', b:'Es un CFDI adicional que el proveedor DEBE emitir cuando una factura es PPD (Pago en Parcialidades o Diferido — pago después del mes de emisión) y tú pagas total o parcial. Documenta folio original, fecha de pago, monto. SIN complemento de pagos = no comprobante de pago = no deducible. Tú lo verificas y lo exiges si no llega', c:'Solo en empresas grandes', d:'Lo emite el comprador',
        ...
        { pregunta:'Compras vino y cerveza para el bar de Fogueira. ¿Cómo manejas el IEPS?',
          a:'Lo retienes y lo enteras', b:'NO retienes IEPS — el IEPS lo pagó el productor/importador y ya viene incluido en tu precio de compra. Tú validas que el proveedor lo desglose en factura, lo separas en tu sistema para conocer el costo real del producto, y verificas que el proveedor sea importador autorizado si es producto importado', c:'Lo absorbes sin verlo', d:'Lo cobras al cliente directo',
        ...
        { pregunta:'¿Qué es la "regla de las 3 manos" en compras y por qué es crítica?',
          a:'Una superstición', b:'Separación obligatoria: (1) quien cotiza y propone proveedor — TÚ; (2) quien autoriza el pago — Mónica (Gerente Administrativo); (3) quien recibe la mercancía y firma evidencia — cocina o churrasca. Si una sola persona hace 2+ funciones hay riesgo de fraude operativo. Te protege a ti también: imposible acusarte de robo si nunca tuviste pago ni mercancía en tus manos', c:'Solo en bancos', d:'Es opcional',
        ...
        { pregunta:'¿Cuáles son KPIs típicos por los que la dirección mide al comprador profesional?',
          a:'Solo el ahorro', b:'Ahorro vs precio anterior (target -2% a -5% ajustado inflación), % compras con 3 cotizaciones (>80%), lead time promedio (<3d), OTIF promedio de proveedores (>90%), # proveedores activos por categoría (>=2 en críticos), % precios validados (sin flag 📍), índice de incidencias (<2%), cumplimiento de reporte semanal (100%)', c:'Cuántas órdenes procesa', d:'No hay KPIs',
  ...
  return {
    ...
    host: [
      ...
      { pregunta:'Un niño de 8 años, ¿qué tarifa aplica?',
        a:'Cortesía', b:'Tarifa niño $249', c:'Tarifa adulto $590', d:'$199', correcta:'b',
        explicacion:'Niños 6-10 pagan tarifa niño $249.' },
      { pregunta:'¿Hasta qué hora puede un cliente cancelar su reserva online?',
        a:'1 hora antes', b:'30 minutos antes', c:'En cualquier momento', d:'No puede cancelar', correcta:'b',
      ...
      { pregunta:'¿Cuál es el cupo máximo de personas reservadas online por servicio?',
        a:'30', b:'50', c:'92', d:'100', correcta:'b',
      ...
      { pregunta:'¿Cuál es el aforo físico máximo del restaurante?',
        a:'50', b:'75', c:'92', d:'120', correcta:'c',
      ...
      { pregunta:'En la pestaña Reservaciones, ¿qué hace el botón verde "✓ Llegó"?',
        a:'Cancela la reserva', b:'Marca la reserva como atendida (cliente ya llegó)', c:'Imprime el ticket', d:'Manda WhatsApp', correcta:'b',
      ...
      { pregunta:'¿Qué hace el botón rojo "Pausar reservas" en la página de Reservaciones?',
        a:'Cancela todas las reservas del día', b:'Bloquea las reservas online del día (no permite que entren nuevas)', c:'Cierra el restaurante para siempre', d:'Pausa las cocinas', correcta:'b',
      ...
      { pregunta:'Si llega un grupo de 2 personas y la bitácora muestra el botón "💡 Mesa 6 (2 pax)", ¿qué pasa al darle click?',
        a:'Imprime un ticket', b:'Asigna automáticamente la mesa 6 (la más chica disponible que les cabe)', c:'Cancela la fila', d:'Muestra un mapa', correcta:'b',
      ...
      { pregunta:'Si los fines de semana cambias del servicio de Desayuno al de Comida, ¿qué se hace?',
        a:'Se sigue en la misma bitácora', b:'Se cierra Desayuno y se abre una NUEVA bitácora de Comida', c:'Se borra todo', d:'No se puede cambiar', correcta:'b',
      ...
      { pregunta:'¿Cuáles son los únicos puestos autorizados para firmar cortesías?',
        a:'Cualquier host', b:'Gerente Administrativo y Gerente de Restaurante', c:'La cajera', d:'Cocina', correcta:'b',
      ...
      { pregunta:'¿Hasta qué hora dura tu sesión iniciada?',
        a:'1 hora', b:'8 horas', c:'Hasta las 3:00 am del día siguiente', d:'Hasta medianoche', correcta:'c',
      ...
      { pregunta:'En iPhone, si después de iniciar sesión te aparece pantalla en blanco, ¿qué haces?',
        a:'Reinicias el iPhone', b:'Tap en el botón "→ Entrar al sistema" que aparece visible', c:'Cierras la sesión', d:'Le pegas al teléfono', correcta:'b',
      ...
      { pregunta:'¿Qué significa que una mesa esté en "En espera" en el plano del salón?',
        a:'Que la mesa está sucia', b:'Que tiene reserva confirmada pero el cliente aún no llega', c:'Que está bloqueada', d:'Que falta capturar adultos', correcta:'b',
      ...
      { pregunta:'Si capturas una cortesía sin asignar quién la autorizó, ¿qué pasa?',
        a:'Nada, se acepta', b:'El sistema la guarda pero la marca como "bandera roja" al cierre', c:'Se borra automáticamente', d:'Se cobra al host', correcta:'b',
      ...
      { pregunta:'¿Qué tipo de reservas NO se confirman automáticamente?',
        a:'Las del fin de semana', b:'Grupos mayores a 10 personas', c:'Las de la noche', d:'Todas se confirman solas', correcta:'b',
      ...
      { pregunta:'¿Cuál es el horario "estelar" o más demandado de Fogueira?',
        a:'8am - 12pm', b:'12pm - 3pm', c:'3pm - 6pm', d:'6pm - 9pm', correcta:'c',
      ...
      { pregunta:'Si trabajas en varios dispositivos al mismo tiempo, ¿qué debes evitar?',
        a:'Las reservas online', b:'Capturar el mismo walk-in en dos dispositivos a la vez (causa duplicados)', c:'Refrescar la página', d:'Iniciar sesión', correcta:'b',
    ...
    cajera: [
      { pregunta:'En el módulo de Conciliación, ¿qué se captura en el "Corte de Caja"?',
        a:'Solo el total en efectivo', b:'El desglose por denominación: cuántos billetes de $1000, $500, $200, etc.', c:'Las propinas', d:'Los tickets cancelados', correcta:'b',
      ...
      { pregunta:'¿Qué tipos de tarjetas se separan en la conciliación?',
        a:'Solo Visa', b:'Débito y Crédito en general', c:'Débito, Mastercard, AMEX, Visa', d:'Todas juntas', correcta:'c',
      ...
      { pregunta:'¿Quiénes son los únicos autorizados para firmar cortesías?',
        a:'Cualquier mesero', b:'Gerente Administrativo y Gerente de Restaurante', c:'La cajera', d:'El admin del sistema', correcta:'b',
      ...
      { pregunta:'¿Cuántos depósitos a tesorería se hacen al final del día?',
        a:'Uno solo con todo', b:'Dos: uno de la venta del día y otro de las comisiones bancarias', c:'Tres', d:'Ninguno', correcta:'b',
      ...
      { pregunta:'¿Qué es el "arqueo ciego" que hace el sistema?',
        a:'Compara automáticamente lo contado contra el efectivo teórico calculado', b:'Es un sorteo', c:'Una multa', d:'Un descuento', correcta:'a',
      ...
      { pregunta:'Si al cierre hay banderas rojas, ¿qué debes hacer?',
        a:'Firmar igual y resolver al día siguiente', b:'NO firmar el cierre y avisar primero a la administradora', c:'Ignorarlas', d:'Borrarlas', correcta:'b',
      ...
      { pregunta:'¿Qué pasa si capturas una cortesía sin folio del ticket?',
        a:'Nada, se acepta', b:'Queda como bandera roja al cierre', c:'Se borra automáticamente', d:'Se cobra a la cajera', correcta:'b',
      ...
      { pregunta:'En el módulo Conciliación, ¿qué hace el botón "Auto-llenar"?',
        a:'Cierra la caja', b:'Consolida automáticamente las bitácoras del backend en los datos de apertura/cierre', c:'Crea una bitácora nueva', d:'Borra todo', correcta:'b',
        explicacion:'Auto-llenar lee las bitácoras del día y consolida (Desayuno→ap, Comida→ci, Buffet→ap).' },
      { pregunta:'¿Qué pasa con las propinas según las reglas Fogueira?',
        a:'Se quedan en caja', b:'Se entregan al final del turno según el reparto definido', c:'Se cobran al patrón', d:'Se anulan', correcta:'b',
        explicacion:'Las propinas se reparten al final del turno según reglas operativas internas.' },
      { pregunta:'¿Cuándo aparece el "Tablero de sellos" en Conciliación?',
        a:'En todas las pestañas', b:'En la sección 05 del Cierre Profundo (lista lo esperado vs lo firmado)', c:'No aparece', d:'Solo en Apertura', correcta:'b',
        explicacion:'El tablero está en la sección 05 (read-only para cajera; admin puede hacer override con motivo).' },
      { pregunta:'¿Cuál es el cupo máximo de personas reservadas online por servicio?',
        a:'25', b:'50', c:'92', d:'100', correcta:'b',
      ...
      { pregunta:'Si una bitácora se cae o se pierde durante el servicio, ¿qué hace el sistema?',
        a:'Pierde todo y debes recapturar', b:'Reintenta guardar de manera infinita y al cerrar el navegador usa sendBeacon como respaldo', c:'No tiene respaldo', d:'Manda email a la cajera', correcta:'b',
      ...
      { pregunta:'¿Qué tarifa aplica un niño de 9 años?',
        a:'Cortesía', b:'Tarifa niño $249', c:'Tarifa adulto $590', d:'Mitad de la tarifa adulto', correcta:'b',
        explicacion:'6-10 años pagan tarifa niño. 0-5 cortesía. 11+ adulto.' },
      { pregunta:'En el corte, ¿cómo se deben entregar los billetes para contar?',
        a:'Mezclados', b:'Por denominación, capturando cantidad de billetes de cada valor', c:'Solo el total', d:'Sin contar', correcta:'b',
      ...
      { pregunta:'Si te equivocas al capturar una cortesía, ¿qué haces?',
        a:'Lo dejas así', b:'Editas la fila desde el detalle (botón ✎) o pides al admin que corrija', c:'Borras toda la bitácora', d:'Llamas a soporte', correcta:'b',
      ...
      { pregunta:'¿Qué significa "sello autenticado" en el cierre?',
        a:'Una calcomanía física', b:'La firma se hace desde el módulo del usuario logueado, imposible firmar por otro', c:'Una contraseña', d:'Un código QR', correcta:'b',
      ...
      { pregunta:'¿Cuál es el horario del Buffet completo (Lun-Jue)?',
        a:'8am - 12pm', b:'1pm - 11pm', c:'12pm - 6pm', d:'10am - 8pm', correcta:'b',
      ...
      { pregunta:'Las cortesías para niños de 0 a 5 años, ¿necesitan autorización del gerente?',
        a:'Sí, siempre', b:'No, son cortesías automáticas por edad', c:'Solo en fin de semana', d:'Solo arriba de 3 niños', correcta:'b',
      ...
      { pregunta:'En el sistema, ¿qué información guarda la columna "ticket" de cortesías?',
        a:'El número de mesa', b:'El folio físico del ticket POS asociado a esa cortesía', c:'La hora de salida', d:'El nombre del cliente', correcta:'b',
      ...
      { pregunta:'Al firmar tu sello de cierre como cajera, ¿qué pasa si te equivocaste?',
        a:'No se puede deshacer nada', b:'Solo el admin puede hacer override con motivo registrado en auditoría', c:'Llamas al soporte técnico', d:'Cierras sesión', correcta:'b',
      ...
      { pregunta:'¿Qué se hace si el cliente paga con vale o cupón especial?',
        a:'Se ignora', b:'Se captura como una forma de pago específica según indique el procedimiento', c:'Se cobra como efectivo', d:'Se rechaza', correcta:'b',
      ...
      { pregunta:'¿Hasta qué hora del día siguiente puede durar tu sesión iniciada?',
        a:'Hasta medianoche', b:'Hasta las 3:00 am del día siguiente', c:'Hasta las 6 am', d:'Hasta las 12 pm del día siguiente', correcta:'b',
      ...
      { pregunta:'Si en el cierre ves que el efectivo contado es MENOR al teórico, ¿qué se llama eso?',
        a:'Sobrante', b:'Faltante', c:'Empate', d:'Diferencia neutra', correcta:'b',
      ...
      { pregunta:'¿Qué significa una bandera roja en la conciliación?',
        a:'Que es feriado', b:'Una alerta del sistema (cortesía sin autorización, diferencia en arqueo, etc.) que requiere explicación', c:'Que la bitácora está cerrada', d:'Que hay descuento', correcta:'b',
      ...
      { pregunta:'¿Cuál es el orden correcto del cierre del día?',
        a:'Firmar y luego conciliar', b:'Capturar corte de caja → tarjetas → cortesías → depósitos → arqueo → revisar banderas → firmar', c:'Solo firmar', d:'Borrar la bitácora primero', correcta:'b',
    ...
    cocina: [
      { pregunta:'¿Cuál es el módulo principal que usa Cocina en el sistema?',
        a:'Bitácora', b:'Charolas', c:'Conciliación', d:'Reservaciones', correcta:'b',
      ...
      { pregunta:'¿Cada cuándo registras una charola en el sistema?',
        a:'Una vez al final del servicio', b:'Cada vez que sacas una charola al buffet', c:'Solo cuando se acaba', d:'Una vez a la semana', correcta:'b',
      ...
      { pregunta:'Si una charola se daña o queda sobrante al final, ¿cómo se registra?',
        a:'Se ignora', b:'Se captura como "Merma"', c:'Se cobra al cliente', d:'Se regala', correcta:'b',
      ...
      { pregunta:'¿Qué información se captura por cada charola?',
        a:'Solo el peso', b:'Qué es (carne, postre, ensalada, etc.) y la cantidad', c:'El precio', d:'El número de mesa', correcta:'b',
      ...
      { pregunta:'¿Para qué sirve la información que captura Cocina?',
        a:'Para multar al equipo', b:'La administradora calcula costo real del servicio y optimiza pedidos al almacén', c:'Para la nómina', d:'No se usa', correcta:'b',
      ...
      { pregunta:'¿En qué momento debe firmarse el sello de apertura de Cocina?',
        a:'Al final del servicio', b:'Al inicio del servicio cuando ya está listo el setup', c:'Nunca', d:'A la mitad del día', correcta:'b',
      ...
      { pregunta:'¿Tu sello de cocina lo puede firmar otra persona desde su cuenta?',
        a:'Sí, cualquiera puede firmar por ti', b:'No, cada quien firma desde SU sesión (sello autenticado por user_id)', c:'Solo el admin', d:'Solo la cajera', correcta:'b',
      ...
      { pregunta:'Si te equivocaste al capturar la cantidad de una charola, ¿qué haces?',
        a:'Borras toda la bitácora', b:'Editas el registro o avisas al admin que corrija', c:'Lo dejas mal', d:'Cierras sesión', correcta:'b',
      ...
      { pregunta:'¿Qué tipos de charola registras típicamente en Fogueira?',
        a:'Solo carnes', b:'Carnes, ensaladas, postres, guarniciones — todo lo que sale al buffet', c:'Solo bebidas', d:'Solo lo que se cobra extra', correcta:'b',
      ...
      { pregunta:'¿Con qué frecuencia recargas el buffet?',
        a:'Cada 4 horas', b:'Según demanda y reposición visual del buffet', c:'Solo al inicio', d:'Una vez al día', correcta:'b',
        explicacion:'Reposición continua según demanda; cada reposición = registro nuevo.' },
      { pregunta:'¿Hasta qué hora dura tu sesión iniciada en el sistema?',
        a:'1 hora', b:'Hasta las 3:00 am del día siguiente', c:'Hasta el medio día', d:'Hasta cerrar el navegador', correcta:'b',
      ...
      { pregunta:'¿Quién es el responsable de capturar las charolas que salen?',
        a:'El cliente', b:'La cajera', c:'La persona de Cocina o Churrasca encargada del buffet', d:'El admin', correcta:'c',
      ...
      { pregunta:'En tu pantalla Charolas, ¿qué hace el botón "+ Nueva charola"?',
        a:'Crea una receta', b:'Abre el formulario para registrar una charola que estás sacando ahora', c:'Cierra el sistema', d:'Imprime un ticket', correcta:'b',
      ...
      { pregunta:'Si el sistema marca tu firma como "pendiente" pero ya pasó la apertura, ¿qué haces?',
        a:'Ignorarlo', b:'Firmar tu sello de apertura desde tu pantalla cuando puedas', c:'Avisar a soporte para que firme por ti', d:'Cerrar el sistema', correcta:'b',
      ...
      { pregunta:'¿Por qué es crítico capturar las mermas correctamente?',
        a:'Para que no se pierda dinero al patrón sin justificar', b:'Para presumir', c:'No es importante', d:'Para sacar de inventario', correcta:'a',
      ...
      { pregunta:'¿Quién accede a la información que tú capturas de charolas?',
        a:'Solo tú', b:'Administradora, gerentes, auditoría — para análisis de costo y operación', c:'Nadie', d:'Solo el cliente', correcta:'b',
      ...
      { pregunta:'Si capturas una charola y por error pones una cantidad muy alta (ejemplo: 50 en lugar de 5), ¿qué pasa?',
        a:'No pasa nada', b:'Distorsiona el análisis de costo; debes corregirla cuanto antes', c:'Se cobra al cliente', d:'Se cancela el sistema', correcta:'b',
      ...
      { pregunta:'En tu pantalla, ¿puedes editar un registro previo del día?',
        a:'No, jamás', b:'Sí, mientras siga el día y queda registro de auditoría', c:'Solo el admin puede', d:'Solo a las 3 am', correcta:'b',
      ...
      { pregunta:'¿Para qué sirve que captures el TIPO de charola (carne, postre, etc.)?',
        a:'Para fines decorativos', b:'Permite analizar consumo por categoría y optimizar pedidos al almacén', c:'No sirve', d:'Para imprimir el menú', correcta:'b',
      ...
      { pregunta:'¿Qué pasa si NO firmas el sello de cocina al inicio del servicio?',
        a:'Nada', b:'Aparece como pendiente en el tablero de sellos y queda como bandera al cierre', c:'Se cobra al patrón', d:'No puedes salir', correcta:'b',
      ...
      { pregunta:'¿Quién puede ver tus capturas en tiempo real?',
        a:'Solo tú', b:'La administradora, gerentes y auditoría desde sus paneles', c:'El cliente', d:'Nadie', correcta:'b',
      ...
      { pregunta:'Si la administradora te dice "captura tal merma con motivo", ¿qué haces?',
        a:'Discutir', b:'Capturar la merma con el motivo en la columna correspondiente y avisar al cierre', c:'Ignorarla', d:'Llamar a otro turno', correcta:'b',
      ...
      { pregunta:'¿Qué dispositivos puedes usar para capturar charolas?',
        a:'Solo PC', b:'Solo iPhone', c:'Cualquier dispositivo con navegador (PC, tablet, smartphone)', d:'Solo tablets', correcta:'c',
      ...
      { pregunta:'Tu rol como cocina, ¿puede ver la conciliación o caja?',
        a:'Sí, todo', b:'No, tu rol está limitado a Charolas y Manual', c:'Solo en fin de semana', d:'Solo si lo pide el patrón', correcta:'b',
      ...
      { pregunta:'¿Cada cuánto debes recertificarte (renovar el examen)?',
        a:'Cada año', b:'Cada 6 meses', c:'Una sola vez de por vida', d:'Cada mes', correcta:'b',
    ...
    churrasca: [
      { pregunta:'¿Cuál es la principal responsabilidad de Churrasca en Fogueira?',
        a:'Cobrar a los clientes', b:'Atender el rodizio: pasar carnes en espada a las mesas y registrar lo que sale', c:'Conciliar caja', d:'Hacer reservas', correcta:'b',
      ...
      { pregunta:'¿En qué módulo del sistema registras las espadas que sales?',
        a:'Bitácora', b:'Charolas', c:'Conciliación', d:'Caja', correcta:'b',
      ...
      { pregunta:'¿Quién más debe firmar el sello de apertura junto contigo?',
        a:'Solo tú', b:'La cocina (los dos chefs firman: cocina y churrasca)', c:'El cliente', d:'El admin', correcta:'b',
      ...
      { pregunta:'¿Tu sello lo puede firmar la cocina por ti?',
        a:'Sí, son del mismo equipo', b:'No, cada quien firma desde SU cuenta autenticada', c:'Sí, si te lo pide', d:'Solo el admin', correcta:'b',
      ...
      { pregunta:'Si una espada no se sirvió completa y queda merma, ¿qué haces?',
        a:'Tirarla sin registrar', b:'Capturarla como Merma con el tipo correspondiente', c:'Cobrarla aparte', d:'Comerla', correcta:'b',
      ...
      { pregunta:'¿Cuál es el horario del rodizio en fin de semana (Comida)?',
        a:'8am - 12pm', b:'1pm - 11pm', c:'12pm - 6pm', d:'8pm - 2am', correcta:'b',
        explicacion:'Vie-Dom comida 1pm-11pm/9pm. Es cuando el rodizio funciona a tope.' },
      { pregunta:'¿Cuál es el aforo físico máximo del restaurante?',
        a:'50', b:'92', c:'120', d:'150', correcta:'b',
      ...
      { pregunta:'En tu pantalla Charolas, al sacar una espada de chorizo, ¿qué capturas?',
        a:'Solo el peso', b:'Tipo (chorizo) + cantidad de espadas', c:'El precio', d:'La mesa exacta', correcta:'b',
      ...
      { pregunta:'Si capturas mal una espada, ¿se puede corregir?',
        a:'No, jamás', b:'Sí, editas o pides al admin que corrija; queda en auditoría', c:'Solo el cliente puede corregir', d:'Solo en horario nocturno', correcta:'b',
      ...
      { pregunta:'¿Por qué es importante registrar TODAS las salidas?',
        a:'No es importante', b:'Es info crítica para costos, control de inventario y análisis del servicio', c:'Solo para el cliente', d:'Para premiar al patrón', correcta:'b',
        explicacion:'Sin registro, no hay control de costo ni de inventario.' },
      { pregunta:'¿Hasta qué hora dura tu sesión iniciada?',
        a:'30 minutos', b:'Hasta las 3:00 am del día siguiente', c:'8 horas exactas', d:'Hasta cerrar el navegador', correcta:'b',
        explicacion:'Día lógico de restaurante: hasta 3 am.' },
      { pregunta:'¿Qué pasa si NO firmas tu sello de apertura?',
        a:'Nada', b:'Aparece como pendiente en el tablero; queda bandera al cierre', c:'Se firma solo', d:'Se cobra al patrón', correcta:'b',
      ...
      { pregunta:'¿Tu rol puede ver el módulo de conciliación de caja?',
        a:'Sí, completo', b:'No, churrasca solo ve Charolas y su Manual', c:'Solo en fines de semana', d:'Solo si Mónica te da acceso', correcta:'b',
      ...
      { pregunta:'Si te das cuenta a media jornada que olvidaste registrar 3 espadas previas, ¿qué haces?',
        a:'Las dejas pasar', b:'Capturas las 3 ahora con la hora correspondiente o con nota; mejor ahora que nunca', c:'Avisas al cliente', d:'Te vas', correcta:'b',
      ...
      { pregunta:'¿Qué información permite el análisis de "qué tipo de carne se consume más"?',
        a:'Adivinanza', b:'Las capturas tuyas y de cocina por tipo y cantidad', c:'El POS', d:'La cámara', correcta:'b',
      ...
      { pregunta:'¿Quién analiza tu data al cierre del día?',
        a:'Nadie', b:'Administradora, gerentes y auditoría', c:'El cliente', d:'Solo el cocinero', correcta:'b',
        explicacion:'Es información operativa-gerencial.' },
      { pregunta:'Tu rol, ¿puede modificar usuarios o tarifas del sistema?',
        a:'Sí', b:'No, eso solo lo hace el admin/gerente administrativo', c:'Solo en navidad', d:'Solo en mayo', correcta:'b',
      ...
      { pregunta:'¿Para qué sirve la columna "responsable_email" en charolas?',
        a:'Decoración', b:'Auditoría: identifica quién registró cada salida', c:'Para enviar publicidad', d:'No sirve', correcta:'b',
      ...
      { pregunta:'Si trabajas en varios dispositivos, ¿hay riesgo de duplicar?',
        a:'No, el sistema es perfecto', b:'Sí, evita capturar la MISMA salida en dos dispositivos a la vez', c:'Solo en iPhone', d:'No aplica', correcta:'b',
      ...
      { pregunta:'En el tablero de sellos del cierre, ¿qué significa que tu sello aparezca en verde "✓ Firmado"?',
        a:'Que estás de vacaciones', b:'Que firmaste correctamente desde tu cuenta autenticada', c:'Una alerta', d:'Pendiente', correcta:'b',
    ...
    encargado_piso: [
      { pregunta:'¿Cuál es la responsabilidad principal del Encargado de Piso?',
        a:'Cocinar', b:'Coordinar la operación en piso: hosts, meseros, fluidez del servicio', c:'Conciliar caja', d:'Gestionar nómina', correcta:'b',
      ...
      { pregunta:'Si una host detecta que el cliente está esperando demasiado, ¿qué haces?',
        a:'Ignorarlo', b:'Acercarte, identificar el bloqueo y coordinar para destrabarlo', c:'Anular la reserva', d:'Echarles la culpa a cocina', correcta:'b',
      ...
      { pregunta:'¿Qué información del sistema necesitas monitorear continuamente?',
        a:'Solo el menú', b:'Plano del salón, reservaciones, bitácora del servicio', c:'Solo cocina', d:'Solo la caja', correcta:'b',
      ...
      { pregunta:'Si una mesa lleva mucho tiempo sin terminar y hay gente esperando, ¿qué pasos sigues?',
        a:'Echarlos a la fuerza', b:'Verificar el plano, hablar con la host, si necesario coordinar otra solución (mesa más chica, espera, etc.)', c:'Cancelar al cliente esperando', d:'No hacer nada', correcta:'b',
      ...
      { pregunta:'¿Quiénes son los únicos que pueden autorizar cortesías?',
        a:'El encargado de piso', b:'Gerente Administrativo y Gerente de Restaurante', c:'Cualquier mesero', d:'La cocina', correcta:'b',
      ...
      { pregunta:'Si un host tiene problemas con el sistema, ¿qué haces tú primero?',
        a:'Llamar a Germán de inmediato', b:'Apoyarlo, verificar si es problema de captura o real, y escalar si es técnico', c:'Anular la jornada', d:'Pedir disculpas al cliente', correcta:'b',
      ...
      { pregunta:'¿Qué hace el botón "Pausar reservas" del módulo Reservaciones?',
        a:'Cancela todas las reservas', b:'Bloquea NUEVAS reservas online del día (lleno o emergencia)', c:'Cierra el restaurante', d:'Pausa al equipo', correcta:'b',
      ...
      { pregunta:'Cuando llega un grupo grande sin reserva, ¿qué consideras?',
        a:'Ignorarlos', b:'Verificar mesas disponibles en plano, pax, capacidad y coordinar con host', c:'Enviarlos a otro restaurante', d:'Cancelar reservas existentes', correcta:'b',
      ...
      { pregunta:'Si la sugerencia automática de mesa propone "Mesa 6 (2 pax)" para un grupo de 4, ¿es correcta?',
        a:'Sí', b:'No: la mesa 6 (2 pax) no cabe a 4 personas; el sistema NO la sugeriría', c:'Sí, si están delgados', d:'Solo en domingo', correcta:'b',
      ...
      { pregunta:'¿Cómo se considera "Desocupada" una mesa?',
        a:'Manualmente', b:'Cuando la host captura la hora_sal de la fila, el estado pasa solo a Desocupada', c:'Cuando se limpia físicamente', d:'Nunca', correcta:'b',
      ...
      { pregunta:'¿Qué pasa si una reserva pasa 10 min de tolerancia y no llega el cliente?',
        a:'Se cancela sola', b:'Queda visualmente atrasada (rojo) pero NO se cancela; el host decide', c:'Se duplica', d:'Se cobra al cliente', correcta:'b',
      ...
      { pregunta:'¿Cuáles son los dos depósitos a tesorería que se hacen al día?',
        a:'Solo uno con todo', b:'Venta del día y comisiones bancarias (separados)', c:'Tres depósitos', d:'Ninguno', correcta:'b',
        explicacion:'Reglas Fogueira: 2 depósitos diferenciados.' },
      { pregunta:'Hasta qué hora del día siguiente dura tu sesión iniciada?',
        a:'medianoche', b:'3:00 am', c:'6:00 am', d:'12:00 pm', correcta:'b',
        explicacion:'Día lógico de restaurante: 3 am del día siguiente.' },
      { pregunta:'En el plano del salón, ¿qué significa "🟡 En espera"?',
        a:'Mesa libre', b:'Reserva confirmada con cliente que aún no llega', c:'Mesa sucia', d:'Cliente comió ya', correcta:'b',
      ...
      { pregunta:'¿Cuál es la prioridad #1 cuando hay banderas rojas en el cierre?',
        a:'Ignorar', b:'Resolverlas/explicarlas antes de firmar el cierre', c:'Borrar la bitácora', d:'Llamar al cliente', correcta:'b',
        explicacion:'Banderas rojas requieren explicación; nunca firmar a la ligera.' },
      { pregunta:'¿Qué tarifa aplica un niño de 7 años?',
        a:'Cortesía', b:'$249 (tarifa niño)', c:'$590 (adulto)', d:'$299 (desayuno adulto)', correcta:'b',
      ...
      { pregunta:'Si la administradora te pide pausar reservas porque están llenos, ¿qué pasa con las que ya están?',
        a:'Se cancelan', b:'Las existentes se respetan; solo se bloquean las NUEVAS', c:'Se borran', d:'Se duplican', correcta:'b',
      ...
      { pregunta:'¿Qué tipo de eventos los gerentes deben aprobar antes de cualquier acción del host?',
        a:'Cancelaciones masivas, cortesías, cambios de tarifa', b:'Cualquier acción mínima', c:'Solo si el cliente paga', d:'Ninguno', correcta:'a',
      ...
      { pregunta:'¿Qué información sirve para predecir cuándo se libera una mesa?',
        a:'Magia', b:'Tiempo promedio histórico (⏱) + tiempo transcurrido en la mesa actual', c:'Ninguna', d:'El menú del día', correcta:'b',
      ...
      { pregunta:'Si necesitas saber cuántas personas están actualmente comiendo, ¿dónde lo ves?',
        a:'Llamando al cliente', b:'En la bitácora del servicio activo o en el mini-resumen del salón', c:'No se puede', d:'En la configuración', correcta:'b',
      ...
      { pregunta:'¿Cómo te enteras si un cliente cancela su reserva online?',
        a:'No te enteras', b:'Aparece un toast/aviso visual en la pantalla del host (y la reserva pasa a "Canceladas")', c:'WhatsApp directo', d:'Email', correcta:'b',
      ...
      { pregunta:'Si tu rol detecta una emergencia (incendio, lesión), ¿qué primero?',
        a:'Capturarla en el sistema', b:'Atender la emergencia física primero; el sistema espera', c:'Llamar a soporte técnico', d:'Cerrar el sistema', correcta:'b',
      ...
      { pregunta:'Si el sistema te avisa "Tu certificación venció", ¿qué pasa?',
        a:'Te bloquean del sistema', b:'Aparece banner suave; puedes seguir trabajando pero debes recertificarte cuando puedas', c:'Te despiden', d:'Te quitan la sesión', correcta:'b',
    ...
    gerente_restaurante: [
      { pregunta:'¿Junto con quién es el Gerente de Restaurante uno de los autorizados a firmar cortesías?',
        a:'Cualquier host', b:'El Gerente Administrativo (Mónica)', c:'La cajera', d:'Un mesero', correcta:'b',
      ...
      { pregunta:'Si una cortesía aparece sin tu nombre como "autoriza", ¿qué pasa?',
        a:'Nada', b:'Es bandera roja al cierre — debe llevar nombre del autorizante', c:'Se cobra al host', d:'Se ignora', correcta:'b',
      ...
      { pregunta:'¿Hasta cuánto antes de la reserva puede cancelar el cliente desde su link?',
        a:'5 minutos', b:'30 minutos', c:'1 hora', d:'No puede cancelar', correcta:'b',
      ...
      { pregunta:'¿Qué información tiene cada sello autenticado en el sistema?',
        a:'Solo el nombre', b:'user_id, email, nombre, rol, fecha/hora exacta y si es override (con motivo)', c:'Solo la hora', d:'Una imagen', correcta:'b',
      ...
      { pregunta:'Si vas a hacer un override admin de un sello, ¿qué te pide el sistema?',
        a:'Nada', b:'Un motivo escrito que queda en auditoría como es_override=true', c:'Tu contraseña', d:'Un código', correcta:'b',
        explicacion:'Motivo obligatorio + queda registrado para auditoría.' },
      { pregunta:'¿Cuáles son los dos depósitos a tesorería del día?',
        a:'Uno solo', b:'Venta del día + comisiones bancarias', c:'Tres depósitos', d:'Ninguno', correcta:'b',
        explicacion:'Reglas Fogueira: 2 depósitos separados.' },
      { pregunta:'En el plano del salón, una mesa con "⏱ ~75 min", ¿qué representa?',
        a:'Su capacidad', b:'Tiempo restante de cierre', c:'Promedio histórico de duración (60 días)', d:'Hora de la próxima reserva', correcta:'c',
        explicacion:'Promedio histórico para predecir disponibilidad.' },
      { pregunta:'En la pestaña Reservaciones, "Pausar reservas", ¿qué hace?',
        a:'Cancela todas', b:'Bloquea NUEVAS reservas online del día', c:'Cierra el restaurante', d:'Anula clientes', correcta:'b',
      ...
      { pregunta:'Si capturas una cortesía, ¿qué información debes asegurar que vaya en la fila?',
        a:'Nombre del autoriza, folio del ticket, motivo', b:'Solo el monto', c:'Solo la mesa', d:'Nada en particular', correcta:'a',
      ...
      { pregunta:'Si los hosts borran una fila, ¿qué se requiere?',
        a:'Solo dar click', b:'Motivo de mínimo 5 caracteres y queda en auditoría (soft-delete)', c:'Tu autorización', d:'Llamar al admin', correcta:'b',
      ...
      { pregunta:'¿Qué pasa con grupos mayores a 10 personas?',
        a:'Se confirman solos', b:'NO se confirman automáticamente; quedan en pendiente_confirmacion para que un host hable con el cliente', c:'Se cobran extra', d:'No pueden reservar', correcta:'b',
      ...
      { pregunta:'¿Cuántos intentos tiene cada usuario en el examen de certificación?',
        a:'1', b:'3', c:'Infinitos', d:'10', correcta:'b',
      ...
      { pregunta:'¿Qué rol está EXENTO del examen de certificación?',
        a:'Host', b:'Cajera', c:'Observador (solo lectura)', d:'Cocina', correcta:'c',
      ...
      { pregunta:'¿Cuántas preguntas hay que contestar correctamente para aprobar el examen?',
        a:'8 de 10', b:'14 de 15 (90%)', c:'10 de 10 (perfecto)', d:'7 de 15', correcta:'b',
        explicacion:'14/15 mínimo (≥90%).' },
      { pregunta:'¿Cuál es la duración de la certificación una vez aprobada?',
        a:'6 meses', b:'1 año', c:'5 años', d:'De por vida', correcta:'a',
      ...
      { pregunta:'Si el sistema sugiere "Mesa 6 (2 pax)" para un grupo de 2, ¿qué tipo de optimización es?',
        a:'Aleatoria', b:'Asignar la mesa más chica disponible que cabe (deja libres las grandes)', c:'Por orden alfabético', d:'Por ID', correcta:'b',
      ...
      { pregunta:'En el sistema, ¿quién puede resetear los intentos de un examen reprobado?',
        a:'Cualquiera', b:'Solo admin / gerente_administrativo', c:'El propio usuario', d:'La cajera', correcta:'b',
      ...
      { pregunta:'Hasta qué hora del día siguiente dura tu sesión activa?',
        a:'Medianoche', b:'3:00 am', c:'6:00 am', d:'10 am', correcta:'b',
        explicacion:'Día lógico restaurante: 3 am.' },
      { pregunta:'En la pestaña Histórico de conciliaciones, ¿qué puedes hacer?',
        a:'Solo ver fecha', b:'Ver KPIs por día, filtrar por fecha/estado, exportar CSV, abrir conciliación de ese día', c:'Modificar registros', d:'Borrar todo', correcta:'b',
        explicacion:'Histórico es read-only con filtros y exportación.' },
      { pregunta:'¿Cuál es el aforo físico máximo de Fogueira?',
        a:'50', b:'92', c:'120', d:'200', correcta:'b',
    ...
    gerente_administrativo: [
      { pregunta:'¿De qué rol heredas todos los privilegios administrativos?',
        a:'Host', b:'Admin', c:'Cajera', d:'Cocina', correcta:'b',
      ...
      { pregunta:'En el panel Admin, ¿qué pestañas puedes administrar?',
        a:'Solo Usuarios', b:'Usuarios, Sucursales, Tarifas vigentes, Configuración, Certificaciones', c:'Solo Tarifas', d:'Ninguna', correcta:'b',
        explicacion:'Acceso completo al panel admin.' },
      { pregunta:'Si un usuario agotó sus 3 intentos del examen, ¿quién puede resetearlos?',
        a:'Nadie', b:'Tú (Gerente Admin) o admin', c:'El propio usuario', d:'Cualquier gerente', correcta:'b',
      ...
      { pregunta:'¿Qué pasa antes de resetear los intentos de un usuario?',
        a:'Nada', b:'Asegurarte de haber realizado capacitación primero', c:'Pagar una multa', d:'Llamar a soporte', correcta:'b',
      ...
      { pregunta:'¿Junto con quién eres autorizado para firmar cortesías?',
        a:'La cajera', b:'Gerente de Restaurante', c:'Cualquier mesero', d:'Tú sola', correcta:'b',
      ...
      { pregunta:'En el módulo Tarifas vigentes, ¿qué se mantiene cuando cambias precios?',
        a:'Solo el más reciente', b:'Todo el histórico (cada cambio es una nueva fila con fecha_desde)', c:'Nada se guarda', d:'Solo la última hora', correcta:'b',
      ...
      { pregunta:'¿Qué pasa cuando creas un usuario nuevo desde Admin?',
        a:'Le mandas tu contraseña', b:'Le asignas rol, das alta y le defines password inicial; él puede cambiarla luego', c:'Solo le mandas un correo', d:'Lo creas sin contraseña', correcta:'b',
      ...
      { pregunta:'¿Cuántos depósitos a tesorería se hacen al día y por qué motivo?',
        a:'Uno solo', b:'Dos: venta del día + comisiones bancarias (separados para análisis)', c:'Tres', d:'Ninguno', correcta:'b',
      ...
      { pregunta:'¿Qué configuración define las mesas del salón con zonas?',
        a:'mesas_salon en formato Salón:1:4|Terraza:30:4', b:'No existe', c:'Solo en Excel', d:'A mano', correcta:'a',
      ...
      { pregunta:'En la pestaña Configuración, ¿qué claves importantes administras?',
        a:'Solo el nombre de la empresa', b:'Cupo por servicio, slot, tolerancia, horario estelar, mesas, gerentes', c:'Solo emails', d:'No hay nada que administrar', correcta:'b',
        explicacion:'Variables operativas core del sistema.' },
      { pregunta:'¿Qué es un "sello override admin" y cuándo se usa?',
        a:'Una calcomanía', b:'Cuando admin firma por otro rol con motivo (queda en auditoría como es_override=true)', c:'Un descuento', d:'Una emoción', correcta:'b',
      ...
      { pregunta:'En el examen de certificación del equipo, ¿cuántas preguntas se sortean del banco?',
        a:'10', b:'15', c:'20', d:'25', correcta:'b',
      ...
      { pregunta:'¿Qué rol está EXENTO del examen?',
        a:'Admin', b:'Host', c:'Observador (solo ve, no edita)', d:'Auditoría', correcta:'c',
      ...
      { pregunta:'Si una cortesía aparece sin nombre del autorizante en una bitácora, ¿qué efecto tiene en el cierre?',
        a:'Ninguno', b:'Bandera roja en conciliación', c:'Se cobra al patrón', d:'Se cierra el sistema', correcta:'b',
      ...
      { pregunta:'En conciliación, las cortesías capturadas en bitácora deben coincidir con:',
        a:'La nada', b:'Los folios del POS y nombres autorizantes', c:'El menú', d:'La nómina', correcta:'b',
      ...
      { pregunta:'En el módulo Histórico de Conciliaciones, ¿qué puedes hacer?',
        a:'Solo ver una fecha', b:'Filtrar rangos, ver KPIs, exportar CSV, abrir cualquier día', c:'Modificar', d:'Eliminar', correcta:'b',
      ...
      { pregunta:'Si te das cuenta que un host borró una fila incorrectamente, ¿se puede recuperar?',
        a:'No, perdido', b:'Sí, soft-delete: la fila sigue en BitacoraFilas con borrada_at + motivo + quién', c:'Solo en lunes', d:'Solo si el cliente reclama', correcta:'b',
      ...
      { pregunta:'¿Cuáles datos personales del usuario quedan asociados a sus capturas?',
        a:'Ninguno', b:'user_id, email y rol al momento de captura — para auditoría', c:'Domicilio', d:'Cuenta bancaria', correcta:'b',
      ...
      { pregunta:'¿Qué tipo de reservas requieren tu confirmación manual antes de aceptarlas?',
        a:'Todas', b:'Grupos mayores a 10 personas', c:'Solo desayunos', d:'Ninguna', correcta:'b',
      ...
      { pregunta:'En la página de inicio, si un usuario tiene certificación vencida, ¿qué le aparece?',
        a:'Bloqueo total', b:'Banner suave amarillo: "Tu certificación venció — Tomar examen"', c:'Una multa', d:'Cierre de sesión', correcta:'b',
      ...
      { pregunta:'¿Qué cumple el sistema de sellos vs firmas tradicionales en papel?',
        a:'Las hace decorativas', b:'Imposibilita firmar por otra persona, captura quién/cuándo y permite override admin con motivo', c:'Las elimina', d:'Las imprime', correcta:'b',
        explicacion:'Autenticación + auditoría + override controlado.' }
    ],
    // ==================== ADMIN ====================
    admin: [
      { pregunta:'¿Quién hereda los privilegios de admin?',
        a:'Cualquiera', b:'Solo Gerente Administrativo', c:'Todos los gerentes', d:'Nadie', correcta:'b',
      ...
      { pregunta:'En el panel Admin, ¿qué se hace en la pestaña Usuarios?',
        a:'Borrar reservas', b:'Crear, editar, activar/desactivar y cambiar contraseña de usuarios', c:'Pedir comida', d:'Imprimir tickets', correcta:'b',
        explicacion:'CRUD completo de usuarios + cambio de password.' },
      { pregunta:'¿Qué pasa al desactivar un usuario?',
        a:'Se borra', b:'No puede iniciar sesión, pero su histórico de capturas se preserva', c:'Pierde su data', d:'Se cobra una multa', correcta:'b',
      ...
      { pregunta:'¿Qué información NO debe estar nunca en el sistema?',
        a:'Nombres y emails', b:'Contraseñas en texto plano (siempre hash)', c:'Roles', d:'Empresa', correcta:'b',
      ...
      { pregunta:'En la pestaña Tarifas vigentes, ¿qué se mantiene cuando cambias precios?',
        a:'Solo el último', b:'Histórico completo (cada cambio = nueva fila con fecha_desde)', c:'Se borra todo', d:'Solo el monto', correcta:'b',
      ...
      { pregunta:'En Configuración, ¿qué clave define el cupo máximo de reservas online por servicio?',
        a:'aforo_fisico', b:'cupo_por_servicio', c:'umbral_grupo_grande', d:'tolerancia', correcta:'b',
      ...
      { pregunta:'En Configuración, formato correcto para mesas con zonas:',
        a:'1,2,3,4', b:'Salón:1:4,2:4|Terraza:30:4,31:4', c:'mesa1=4', d:'JSON aleatorio', correcta:'b',
      ...
      { pregunta:'¿Qué parámetro define en cuántos minutos se considera atraso una reserva?',
        a:'cupo_por_servicio', b:'tolerancia_minutos (default 10)', c:'slot_minutos', d:'aforo_fisico', correcta:'b',
      ...
      { pregunta:'En BitacoraFilas, cuando se "elimina" una fila, ¿qué pasa físicamente?',
        a:'Se borra para siempre', b:'Soft-delete: borrada_at + motivo + borrada_por; el row sigue para auditoría', c:'Se duplica', d:'Se exporta', correcta:'b',
      ...
      { pregunta:'¿Qué endpoint backend calcula el promedio de ocupación de cada mesa?',
        a:'mesa_get', b:'ocupacion_promedio_mesas (60 días, filtros 5-240 min)', c:'reserva_get', d:'tarifa_get', correcta:'b',
      ...
      { pregunta:'En el módulo de sellos, ¿qué hace un override admin?',
        a:'Borra el sello', b:'Permite firmar por otro rol con motivo registrado en auditoría (es_override=true)', c:'Cambia la hora', d:'Cancela el cierre', correcta:'b',
      ...
      { pregunta:'En la hoja Sellos, ¿qué columnas garantizan auditoría?',
        a:'solo nombre', b:'user_id, user_email, user_nombre, user_rol, sellado_at, es_override, motivo_override', c:'solo fecha', d:'una contraseña', correcta:'b',
      ...
      { pregunta:'En el endpoint examen_iniciar, ¿qué pasa si el banco tiene menos de 15 preguntas activas para un rol?',
        a:'Continúa con menos', b:'Devuelve error: "Banco insuficiente"', c:'Inventa preguntas', d:'Llama al admin', correcta:'b',
      ...
      { pregunta:'En conciliación, ¿qué pasa con cortesías sin autorizante?',
        a:'Pasan', b:'Bandera roja al cierre — debe llevar nombre del autoriza', c:'Se borran solas', d:'No importan', correcta:'b',
        explicacion:'Cortesía sin firma = bandera roja en arqueo.' },
      { pregunta:'¿Cuáles son los dos depósitos a tesorería diarios?',
        a:'Uno solo', b:'Venta del día + comisiones bancarias (separados)', c:'Tres distintos', d:'No se hacen', correcta:'b',
        explicacion:'Reglas Fogueira: separación contable.' },
      { pregunta:'Cuando el admin resetea los intentos de un usuario, ¿qué se guarda?',
        a:'Nada', b:'reseteado_por (email del admin) y reseteado_at en cada intento previo no aprobado', c:'Solo la fecha', d:'El nombre', correcta:'b',
      ...
      { pregunta:'¿Qué hace matchSucursal(filaSuc, querySuc) en el backend?',
        a:'Cierra sucursales', b:'Si fila tiene sucursal_id vacío, aplica a CUALQUIER sucursal de la empresa (global). Si tiene un id concreto, solo a esa', c:'Borra la sucursal', d:'No existe', correcta:'b',
      ...
      { pregunta:'¿Qué pasa cuando se cambia la contraseña de un usuario desde admin?',
        a:'Se imprime', b:'Se hashea y guarda en password_hash; el usuario debe usar la nueva al siguiente login', c:'Se manda por email automático', d:'Se queda en plano', correcta:'b',
        explicacion:'Hash + uso inmediato.' },
      { pregunta:'¿Hasta qué hora del día siguiente dura un token de sesión?',
        a:'Hasta cerrar el navegador', b:'Hasta las 3:00 am del día siguiente (día lógico restaurante)', c:'1 hora', d:'Indefinido', correcta:'b',
      ...
      { pregunta:'¿Qué tabla guarda el banco maestro de preguntas del examen?',
        a:'Bitacoras', b:'Examenes (cols: id, rol, pregunta, opcion_a-d, correcta, explicacion, activa, creada_at)', c:'Tarifas', d:'Conciliaciones', correcta:'b',
    ...
    auditoria: [
      { pregunta:'¿Cuál es el principal objetivo del rol Auditoría en el sistema?',
        a:'Operar caja', b:'Revisar registros, sellos y conciliaciones para verificar integridad y controles', c:'Cocinar', d:'Capturar reservas', correcta:'b',
      ...
      { pregunta:'¿Qué tipo de "delete" usa el sistema para filas de bitácora?',
        a:'Borrado físico permanente', b:'Soft-delete (borrada_at + motivo + borrada_por)', c:'No permite borrar', d:'Solo en lunes', correcta:'b',
        explicacion:'Soft-delete preserva auditoría completa.' },
      { pregunta:'¿Qué información identifica unívocamente quién firmó un sello?',
        a:'Una imagen', b:'user_id + user_email + user_nombre + user_rol + sellado_at', c:'Solo el nombre', d:'Solo la hora', correcta:'b',
        explicacion:'Identidad funcional + momento exacto.' },
      { pregunta:'Si un sello tiene es_override=true, ¿qué significa?',
        a:'Que falló', b:'Admin firmó por otro rol con motivo registrado en motivo_override', c:'Que es viejo', d:'Que se duplicó', correcta:'b',
        explicacion:'Override es excepción documentada — auditable.' },
      { pregunta:'¿Qué bandera roja debes vigilar en conciliaciones?',
        a:'Solo el clima', b:'Cortesías sin autorización, diferencias en arqueo, sellos pendientes', c:'Las recetas', d:'No hay banderas', correcta:'b',
      ...
      { pregunta:'¿Cuáles son los dos depósitos a tesorería que esperas ver al cierre?',
        a:'Uno solo', b:'Venta del día + comisiones bancarias (separados)', c:'Cuatro depósitos', d:'Ninguno', correcta:'b',
      ...
      { pregunta:'Si una cortesía no tiene "autoriza" registrado, ¿qué tipo de hallazgo es?',
        a:'Sin importancia', b:'Hallazgo de control: falta firma del autorizante (bandera roja)', c:'Por culpa del cliente', d:'Sin clasificar', correcta:'b',
      ...
      { pregunta:'En la hoja BitacoraFilas, ¿cómo identificas filas eliminadas?',
        a:'No se identifican', b:'Tienen valor en borrada_at + borrada_motivo + borrada_por', c:'Tienen color rojo', d:'Por la hora', correcta:'b',
        explicacion:'Soft-delete con campos llenos.' },
      { pregunta:'En auditoría, ¿qué tablas son tu fuente principal de evidencia?',
        a:'Solo Usuarios', b:'Bitacoras + BitacoraFilas + Sellos + Conciliaciones + Charolas', c:'Solo Reservas', d:'Solo Tarifas', correcta:'b',
        explicacion:'Conjunto operativo donde se capturan transacciones y sellos.' },
      { pregunta:'¿Quiénes son los únicos autorizados para firmar cortesías?',
        a:'Cualquier mesero', b:'Gerente Administrativo y Gerente de Restaurante', c:'La cajera', d:'Auditoría', correcta:'b',
      ...
      { pregunta:'¿Qué endpoint te lista todas las certificaciones del equipo?',
        a:'examen_iniciar', b:'certificaciones_list (admin/auditoria/gerentes)', c:'reserva_get', d:'no existe', correcta:'b',
      ...
      { pregunta:'¿Qué pasa si un usuario reprueba 3 veces el examen?',
        a:'Lo despiden', b:'Queda "bloqueado_por_intentos"; admin debe resetear tras capacitación', c:'Se borran los intentos solos', d:'Se firma solo', correcta:'b',
      ...
      { pregunta:'¿Cuántas preguntas presenta el examen?',
        a:'10', b:'15 sorteadas del banco', c:'25', d:'5', correcta:'b',
        explicacion:'15 al azar; rotación entre intentos.' },
      { pregunta:'¿Qué rol está exento del examen?',
        a:'Admin', b:'Observador (solo lectura)', c:'Auditoría', d:'Cajera', correcta:'b',
        explicacion:'Observador no opera, no requiere certificarse.' },
      { pregunta:'¿Vigencia de la certificación?',
        a:'1 mes', b:'6 meses', c:'1 año', d:'Permanente', correcta:'b',
      ...
      { pregunta:'¿Qué columna identifica si un usuario pasó override en su sello?',
        a:'es_override (booleano)', b:'No existe', c:'estado', d:'fecha', correcta:'a',
        explicacion:'Flag bool y motivo asociado para auditoría.' },
      { pregunta:'¿Hasta qué hora dura una sesión iniciada?',
        a:'1 hora', b:'3:00 am del día siguiente (día lógico restaurante)', c:'12 horas exactas', d:'Indefinido', correcta:'b',
        explicacion:'Cubre operación completa hasta cierre.' },
      { pregunta:'En el reporte Histórico de Conciliaciones, ¿qué puedes hacer?',
        a:'Modificar registros', b:'Filtrar fechas/estado, ver KPIs, exportar CSV — solo lectura', c:'Borrar conciliaciones', d:'Reabrir', correcta:'b',
      ...
      { pregunta:'¿Cuántos depósitos a tesorería esperarías ver y por qué motivo?',
        a:'Uno solo', b:'Dos: venta del día y comisiones bancarias (separación contable)', c:'Tres', d:'Variable', correcta:'b',
        explicacion:'Regla Fogueira para análisis.' },
      { pregunta:'En los sellos, ¿quién puede firmar por otro?',
        a:'Cualquiera', b:'Solo admin/g_admin con override y motivo (excepción auditable)', c:'Cualquier gerente', d:'La cajera', correcta:'b',
      ...
      { pregunta:'¿Qué efecto tiene en el sistema una bandera roja sin explicar?',
        a:'Ninguno', b:'Visible al cierre — auditoría puede observar y exigir explicación', c:'Multas automáticas', d:'Cierre del local', correcta:'b',
      ...
      { pregunta:'En el sistema, ¿quién puede MODIFICAR una conciliación ya cerrada?',
        a:'Cualquiera', b:'Solo admin con override y motivo (auditable)', c:'Auditoría', d:'Nadie', correcta:'b',
      ...
      { pregunta:'¿Cuál es el rol del banner "Tu certificación venció" en inicio?',
        a:'Bloquear acceso', b:'Recordatorio suave no bloqueante; el usuario sigue trabajando', c:'Multa', d:'Cierre forzado', correcta:'b',
    ...
    comprador: [
      ...
      { pregunta:'¿En qué pantalla del sistema editas precios de ingredientes?',
        a:'Bitácora', b:'Recetas y costeo, tab Ingredientes (edición inline)', c:'Conciliación', d:'Charolas',
        correcta:'b', explicacion:'Tab Ingredientes es tu cabina de mando. Búsqueda y edición rápida de precio.' },
      { pregunta:'¿Tu edición de precio es inmediata o queda como propuesta esperando aprobación?',
        a:'Es propuesta como Sergio/Marcos', b:'INMEDIATA. A diferencia del Modelo B, tú editas precios directo y el cambio se propaga al instante a todas las recetas que usan ese ingrediente', c:'Solo en lunes', d:'Mónica aprueba',
      ...
      { pregunta:'¿Qué significa el flag 📍 en un ingrediente?',
        a:'Que es nuevo', b:'Que el precio es estimado (no validado contra factura). Hay 297 estimados pendientes de validar', c:'Que se acabó', d:'Que está caro',
        correcta:'b', explicacion:'📍 = costo aproximado, no exacto. Tu misión es ir reduciendo ese número.' },
      { pregunta:'¿Cuándo quitas el flag 📍 de un ingrediente?',
        a:'Nunca', b:'Cuando ya tienes el precio confirmado por factura o proveedor real', c:'Cada lunes', d:'Cuando Mónica lo pide',
        correcta:'b', explicacion:'Quitar 📍 = precio validado. Reducir los 297 estimados es trabajo de fondo.' },
      { pregunta:'Sergio te pide usar "queso provoleta" que no existe en el catálogo. ¿Qué haces?',
        a:'Le dices que no se puede', b:'Botón "+ Nuevo ingrediente" en tab Ingredientes. Llenas nombre, categoría, unidad, precio (estimado si no tienes factura). Queda disponible para que Sergio lo use', c:'Lo dejas pendiente', d:'Le pides a Mónica',
        correcta:'b', explicacion:'Crear ingredientes nuevos es tu rol. Sergio propone su uso; tú lo das de alta.' },
      { pregunta:'Capturaste un precio mal por un dedazo (pusiste $1,800 en lugar de $180). ¿Qué pasa?',
        a:'Nada grave', b:'El error se propaga inmediato a todas las recetas que usan ese ingrediente — las verás 10× más caras en Rentabilidad. Lo corriges de inmediato y verificas el tab Rentabilidad', c:'Se cancela solo', d:'Se llama al proveedor',
      ...
      { pregunta:'¿Qué criterio NO sirve para priorizar la validación de los 297 estimados?',
        a:'Volumen de uso', b:'Orden alfabético — ese criterio NO ayuda. Lo correcto es priorizar por volumen de uso, precio relativo (caros) y volatilidad (precios que cambian seguido)', c:'Precio relativo', d:'Volatilidad',
        correcta:'b', explicacion:'Impacto = volumen × precio × volatilidad. Alfabético no aporta valor.' },
      { pregunta:'Validas un ingrediente y descubres que el precio real es $150/kg cuando estaba estimado en $50/kg (3×). ¿Qué haces?',
        a:'Cambias en silencio', b:'Capturas el precio real, quitas flag 📍, y avisas a Mónica de inmediato porque las recetas que lo usaban estaban con costo muy subestimado y puede afectar decisiones de menú', c:'No lo cambias', d:'Esperas al fin de mes',
        correcta:'b', explicacion:'Diferencia >30% = mal estimado severo. Mónica necesita reaccionar.' },
      { pregunta:'¿Cuál es el ritmo razonable para validar los 297 estimados?',
        a:'Todo en una semana', b:'10-20 por semana, en paralelo al día a día. En ~6 meses se completan todos. Trabajo de fondo constante', c:'No validarlos', d:'Solo cuando alguien lo pide',
        correcta:'b', explicacion:'Trabajo de fondo. Apretar todo de un jalón no es realista; abandonar tampoco.' },
      { pregunta:'¿Qué incluye tu reporte semanal a Mónica?',
        a:'Solo número de cambios', b:'Resumen ejecutivo, subidas notables (>10%), bajadas, estimados validados, ingredientes nuevos, negociaciones en curso, riesgos. Cada subida con precio anterior/nuevo, variación % y recetas afectadas', c:'Lista de quejas', d:'Solo precios',
        correcta:'b', explicacion:'Reporte = visión consolidada accionable. Inteligencia operativa que la dirección puede actuar.' },
      { pregunta:'¿Cuándo NO debes esperar al reporte semanal y avisar inmediato?',
        a:'Nunca', b:'Cuando hay un cambio grande (subida >30% en ingrediente importante). WhatsApp directo a Mónica sin esperar al lunes', c:'Cualquier cambio', d:'Solo si Mónica pregunta',
        correcta:'b', explicacion:'Materialidad define urgencia. Cambios chicos esperan al reporte; grandes son inmediatos.' },
      { pregunta:'¿Editas recetas (cantidades, ingredientes, instrucciones)?',
        a:'Sí, todas', b:'No. Tu rol edita PRECIOS de ingredientes. Las recetas las modifican Sergio/Marcos con Modelo B (proponen → Mónica aprueba). Tú las ves en lectura', c:'Solo los lunes', d:'Solo si te lo piden',
        correcta:'b', explicacion:'Separación de funciones: precios eres tú; recetas son los chefs.' },
      { pregunta:'¿Tu rol puede ver el módulo de conciliación de caja?',
        a:'Sí, completo', b:'No. Comprador solo ve Recetas y costeo, Mi curso, Mi manual', c:'Solo en fines de semana', d:'Solo si Mónica te da acceso',
        correcta:'b', explicacion:'Por separación de funciones, tu acceso es estrictamente al catálogo de costos.' },
      { pregunta:'¿Cuál es el peor error al validar estimados?',
        a:'Validar contra factura', b:'Quitar el flag 📍 sin actualizar el precio (peor: dice "validado" pero no lo es), validar al ojo sin factura, validar todos rápido sin verificar', c:'Avisar a Mónica', d:'Capturar precio real',
      ...
      { pregunta:'En el futuro habrá un importador Excel del POS. ¿Qué hará?',
        a:'Reemplazará al comprador', b:'Te permitirá subir un Excel exportado de SoftRestaurant y aplicar masivamente cambios de precios al sistema, después de que TÚ revises y confirmes. Convierte 100 cambios manuales en 5 minutos vs 1 hora', c:'Solo lo verás tú', d:'No hace nada',
        correcta:'b', explicacion:'Importador acelera lo masivo, pero tú sigues siendo el filtro de control.' },
      { pregunta:'¿Qué puedes hacer AHORA para que el importador funcione bien cuando llegue?',
        a:'Nada, esperar', b:'Mantener nombres consistentes entre Fogueira y SoftRestaurant, mantener unidades consistentes (kg/gr/L/ml), documentar tu workflow actual', c:'Construirlo tú', d:'Borrar el catálogo',
        correcta:'b', explicacion:'Preparar el terreno hoy = importador eficaz mañana. Consistencia entre sistemas es responsabilidad operativa.' },
      { pregunta:'¿Cuál es el TIPO de información de respaldo que dejas como "notas" en un ingrediente?',
        a:'Cualquier cosa', b:'Marca, proveedor, lote, número de factura. Ej: "Validado contra factura · Proveedor X · Mayo 2026"', c:'No se permiten notas', d:'Solo el precio',
        correcta:'b', explicacion:'Trazabilidad: las notas permiten reconstruir de dónde vino el precio cuando se audita.' },
      { pregunta:'¿Hasta qué hora dura tu sesión iniciada?',
        a:'1 hora', b:'Hasta las 3:00 am del día siguiente (día lógico restaurante)', c:'12 horas', d:'Hasta cerrar el navegador',
        correcta:'b', explicacion:'Día lógico de restaurante: cubre operación completa hasta cierre.' },
      { pregunta:'¿Cada cuánto necesitas renovar tu certificación?',
        a:'Cada año', b:'Cada 6 meses; el sistema te avisa cuando se acerque o se pase la fecha', c:'Una sola vez', d:'Cada mes',
      ...
      { pregunta:'¿Cuál es la diferencia clave entre comprador transaccional y estratégico?',
        a:'Solo el sueldo', b:'El transaccional procesa pedidos como se le piden; el estratégico anticipa, negocia, optimiza el costo total y mitiga riesgo. Métricas distintas: # de órdenes vs ahorro/OTIF/calidad/riesgo', c:'No hay diferencia', d:'El horario',
        correcta:'b', explicacion:'El estratégico genera valor; el transaccional ejecuta tareas. Tu rol en Fogueira es estratégico.' },
      { pregunta:'¿Cuáles son las "5 R" clásicas de las compras corporativas?',
        a:'Rápido, rico, rojo, raro, recio', b:'Right product, Right quality, Right quantity, Right time, Right price (producto, calidad, cantidad, tiempo y precio correctos)', c:'Recibir, revisar, reclamar, reportar, repetir', d:'No existe ese marco',
        correcta:'b', explicacion:'Las 5 R aseguran que la compra sea integralmente correcta. Falla cualquiera y la compra fracasó (aunque el precio fuera bueno).' },
      { pregunta:'Un proveedor te ofrece 2% de "comisión personal" si lo eliges. ¿Qué haces?',
        a:'Lo aceptas en privado', b:'Le respondes que no aceptas comisiones personales y le pides que aplique ese 2% como descuento en factura. Reportas por escrito a Mónica el mismo día. Es cohecho entre particulares (delito) y causal de despido sin liquidación', c:'Lo aceptas si es chico', d:'Pides 5%',
        correcta:'b', explicacion:'Aceptar es delito federal + despido inmediato sin liquidación. Reportar por escrito te protege.' },
      { pregunta:'¿Por qué la regla es 3 cotizaciones (no 1, 2 o 5)?',
        a:'Es número mágico', b:'1 no da referencia; 2 puede ser colusión; 5+ desperdicia tiempo y desincentiva al proveedor. 3 es el equilibrio internacional (ISO 9001, COSO) entre rigor y costo de proceso', c:'Es decisión personal', d:'Lo dice la cocina',
      ...
      { pregunta:'¿Qué es el TCO (Total Cost of Ownership) y por qué importa más que el precio unitario?',
        a:'Es solo precio', b:'Costo total real de comprarle a un proveedor: precio + costo de no-calidad + costo de retraso (compra pánico, horas extra) + costo de servicio (gestión problemas) + costo financiero (días de crédito). Captura lo que el precio unitario esconde — un proveedor barato puede ser más caro en TCO si falla', c:'Una marca', d:'Algo solo de empresas grandes',
        correcta:'b', explicacion:'TCO honesto suele derrotar al precio unitario. El "ahorro" del más barato es ficción cuando incluyes costo del fallo.' },
      { pregunta:'¿Qué es el OTIF y cuál es el target razonable para un proveedor de Fogueira?',
        a:'No se mide', b:'On-Time In-Full = entregas a tiempo Y completas / total de entregas. Target Fogueira: >90%. Por debajo de 80%, el proveedor cuesta más en compras pánico y horas extra de lo que aparenta', c:'Una hormona', d:'Solo en exportación',
        correcta:'b', explicacion:'OTIF es indicador clave de confiabilidad. Llevar registro objetivo permite conversación profesional con el proveedor.' },
      { pregunta:'¿Qué es el BATNA y por qué es crítico antes de toda negociación?',
        a:'Una técnica de presión', b:'Best Alternative to Negotiated Agreement: tu plan B real si esta negociación no se cierra. La regla de 3 cotizaciones ES tu BATNA — sin alternativas reales en mano, estás a merced del proveedor', c:'Acrónimo legal', d:'Software',
        correcta:'b', explicacion:'Sin BATNA tu poder de negociación es mínimo. Por eso las 3 cotizaciones no son burocracia: son tu palanca real.' },
      { pregunta:'¿Qué documentos fiscales SIEMPRE pides antes de dar de alta a un proveedor?',
        a:'Solo el RFC', b:'Constancia de Situación Fiscal vigente; Opinión de Cumplimiento positiva (32D); verificación de NO estar en lista 69-B SAT; carátula bancaria validada; datos de contacto. Renovar 32D cada 6 meses', c:'Solo el nombre', d:'Nada',
        correcta:'b', explicacion:'Sin estos documentos las facturas pueden ser no deducibles. Lista 69-B definitivos = facturas inválidas + corresponsabilidad fiscal.' },
      { pregunta:'Una factura es PPD (Pago en Parcialidades o Diferido). Pagas el monto total. ¿Qué documento adicional necesitas del proveedor?',
        a:'Ninguno', b:'Complemento de pagos (CFDI adicional) que documenta folio de la factura original, fecha de pago, monto y forma de pago real. Sin complemento = no comprobante de pago = no deducible. Tú lo verificas y lo exiges si no llega', c:'Solo el WhatsApp del proveedor', d:'Una nota de venta',
      ...
      { pregunta:'¿Por qué el principio de "3 manos" (cotización / autorización / recepción) en compras?',
        a:'Es burocracia', b:'Si una sola persona controla las 3 funciones tiene carta blanca para defraudar. Separación: tú cotizas y propones; Mónica autoriza pago; cocina/churrasca recibe y firma. Te protege a TI también: imposible acusarte de robo si nunca tuviste el pago ni la mercancía en tus manos', c:'Solo aplica en bancos', d:'Es opcional',
        correcta:'b', explicacion:'Control interno básico. Es protección de la empresa Y del comprador.' },
      { pregunta:'Tu primo tiene una distribuidora y quieres incluirlo en cotizaciones. ¿Práctica correcta?',
        a:'Excluirlo siempre', b:'Declararlo POR ESCRITO a Mónica como conflicto de interés ANTES de iniciar la cotización. Pedir que ella tome la decisión final con base en las 3 cotizaciones objetivas', c:'Incluirlo y darle el contrato', d:'Mentir',
...
function generarToken(usuario) {
  ...
  var raw = usuario.id + '|' + String(usuario.email).toLowerCase() + '|' + diaLogico;
...
function handleBitacoraList(p) {
  ...
  return { ok:true, bitacoras: rowsToObjects(getSheet('Bitacoras')).filter(function(b){
    if (b.empresa_id !== u.empresa_id) return false;
    if (sucursal_id && b.sucursal_id !== sucursal_id) return false;
    if (fecha && fechaToString(b.fecha) !== fecha) return false;
    return true;
  }).map(function(b){
    return { id:b.id, folio:b.folio, fecha:fechaToString(b.fecha), servicio:b.servicio, host_email:b.host_email, estado:b.estado,
...
function handleBitacoraFilasList(p) {
  ...
  var filas = rowsToObjects(sheet).filter(function(f){
    ...
    if (f.borrada_at) return false; // excluir soft-deleted
    return true;
  });
  filas.sort(function(a,b){ return parseFloat(a.idx) - parseFloat(b.idx); });
  // Calcular max_ts para que el cliente pueda hacer polling incremental después
  var maxTs = 0;
  ...
  return { ok:true, max_ts: maxTs, filas: filas.map(function(f){
    ...
    datos._id = f.id;
    datos._idx = parseFloat(f.idx);
    // Metadatos de auditoría/sync (NO confundir con campos de datos del usuario)
    var ts = (f.actualizada_at && f.actualizada_at.getTime) ? f.actualizada_at.getTime() : new Date(f.actualizada_at).getTime();
...
function handleBitacoraFilasSync(p) {
  ...
  todas.forEach(function(f){
    ...
    if (sinceMs > 0 && maxFila <= sinceMs) return; // ningún cambio relevante
    if (f.borrada_at) {
...
function handleBitacoraFilaSave(p) {
  ...
  var ahora = new Date();
  var existing = fila_id ? rowsToObjects(sheet).find(function(f){ return f.id === fila_id; }) : null;
  if (existing) {
...
function handleBitacoraFilaDelete(p) {
  ...
  var sheet = asegurarHoja('BitacoraFilas', BITACORA_FILAS_COLS);
  var existing = rowsToObjects(sheet).find(function(f){ return f.id === fila_id; });
  if (!existing) return { ok:false, error:'Fila no encontrada' };
  var row = existing._row;
...
var ROLES_EXENTOS_CERTIFICACION = ['observador'];
...
function handleConciliacionesList(p) {
  ...
  var resultado = conciliaciones.map(function(c){
    ...
    // Calcular KPIs principales del payload
    var kpis = calcularKpisConciliacion(payload);
    return {
      id: c.id,
...
function handleCharolasList(p) {
  ...
  return { ok:true, charolas: charolas.map(function(c){
    return { id:c.id, fecha:fechaToString(c.fecha), hora:c.hora, area:c.area, tipo:c.tipo,
...
function handleCharolasCreate(p) {
  ...
  // Solo para área churrasca por ahora (la cocina lo conectamos en futuro).
  var descuento = null;
  if (receta_id && area === 'churrasca' && (tipo === 'charola' || tipo === 'reposicion' || tipo === 'merma')) {
